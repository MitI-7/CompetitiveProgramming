...
struct DNAString {
    ...
    int solve(int period, const string &S) {
        int ans = 0;
        FOR(i, 0, period) {
            map<int, int> freq;
            for (int j = i; j < LEN(S); j += period) {
                freq[S[j]]++;
            }
    ...
    int minChanges(int _maxPeriod, vector<string> _dna) {
        maxPeriod = _maxPeriod, dna = _dna;
        string S;
        FOE(x, dna) {
            S += x;
        }
...
// CUT begin
ifstream data("../SRM 396_DIV1_250.sample");
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "DNAString (250 Points)" << endl << endl;
    }
