...
struct ColorfulMazeTwo {
    vector<string> maze;
    vector<int> trap;
    double getProbability(vector<string> _maze, vector<int> _trap) {
        maze = _maze, trap = _trap;
        int H = LEN(maze);
        int W = LEN(maze[0]);

        auto dp = make_v<double>(H, W, (1 << 7) + 1);
        fill_v(dp, 0);
        int sy = -1, sx = -1;
        int gy = -1, gx = -1;
        FOR(y, 0, H) {
        ...
        que.emplace(make_tuple(1.0, sy, sx, 0));
        while (not que.empty()) {
            double p;
            int y, x, s;
            tie(p, y, x, s) = que.top(); que.pop();
            if (dp[y][x][s] >= p) {
                continue;
            }
            dp[y][x][s] = p;
            ...
            FOR(i, 0, 4) {
                int ny = y + dy4[i];
                int nx = x + dx4[i];
                if (inside(ny, nx, H, W) and maze[ny][nx] != '#') {
                    const auto c = maze[ny][nx];
                    if (c == '.' or c == '!') {
                        que.emplace(make_tuple(p, ny, nx, s));
                    }
                    else if (c != '$' and c != '!') {
                        int a = c - 'A';
                        if (is_bit_on(s, a)) {
                            ...
                            que.emplace(make_tuple(p * (1.0 - trap[a] / 100.0), ny, nx, bit_set(s, a, 1)));
                        }
                    }
                }
            }
...
// CUT begin
ifstream data("../SRM 464_DIV2_1000.sample");
