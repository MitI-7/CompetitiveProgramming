...
struct SilverDistance {
    ...
    void dfs(int y, int x, vector<vector<int>> &field) {
        vector<vector<int>> d = {{-1, +1}, {0,1}, {1,1}, {-1,-1}, {+1,-1}};
        FOR(i, 0, 5) {
            int ny = y + d[i][1];
            int nx = x + d[i][0];
    ...
    int solve(int sx, int sy, int gx, int gy) {
        ...
        const int y_diff = abs(gy);
        const int x_diff = abs(gx);
        if (sx == gx) {
            // 上
            if (gy > sy) {
                return y_diff;
            ...
            // 下
                ...
                return y_diff + (y_diff % 2 == 0 ? 0 : 2);
            }
        }

        // 右
        if (sy == gy) {
            return x_diff + (x_diff % 2 == 0 ? 0 : 1);
        }
        // 右上
        if (gy > sy) {
            if (y_diff >= x_diff) {
        ...
        if (sy > gy) {
            if (y_diff >= x_diff) {
                int down = y_diff + (y_diff % 2 == 0 ? 0 : 2);

                if (y_diff % 2 == 1 and x_diff % 2 == 1) {
                    return down - 2;
                }
                if (y_diff % 2 == 0 and x_diff % 2 == 1) {
                    return down + 2;
                }
                return down;
            }
    ...
    int minSteps(int _sx, int _sy, int _gx, int _gy) {
        return solve(_sx, _sy, _gx, _gy);
    }
};

// CUT begin
ifstream data("../SRM 456_DIV1_250.sample");
