...
struct MagicWords {
    ...
    bool check(string &s) {
        ...
        RollingHash rh(s + s);
        auto base = rh.hash(0, N);
        int num = 0;
        FOR(i, 0, LEN(s)) {
            num += (base == rh.hash(i, i + N));
        }
        return num == K;
    }

    int count(vector<string> _S, int _K) {
        S = _S, K = _K;

        int ans = 0;

        auto v = make_v<int>(LEN(S));
        iota(ALL(v), 0);
        sort(v.begin(), v.end());
        do {
...
// CUT begin
ifstream data("../SRM 433_DIV1_250.sample");
