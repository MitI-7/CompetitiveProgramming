...
struct Undo {
    ...
    string getText(vector<string> _commands, vector<int> _time) {
        commands = _commands, time = _time;
        int N = LEN(commands);
        auto exec = make_v<bool>(N);
        auto range = make_v<pair<int, int>>(0);
        for (int i = N - 1; i >= 0; --i) {
            int count = 0;
            FOE(p, range) {
                if (p.first <= time[i] and time[i] <= p.second) {
                    count++;
                    break;
                }
            }
            ...
            if (commands[i][0] == 'u') {
                auto t = stol(split(commands[i], " ")[1]);
                range.emplace_back(make_pair(time[i] - t, time[i] - 1));
            }
            else {
                exec[i] = true;
            }
...
// CUT begin
ifstream data("../SRM 419_DIV1_250.sample");
