...
struct IdealString {
    ...
    string make_right(string &left) {
        auto occ = make_v<int>(30);
        FOR(i, 0, LEN(left)) {
            if (occ[left[i] - 'A'] == 0) {
                occ[left[i] - 'A'] = i + 1;
            }
    ...
    string construct(int _length) {
        ...
        FOR(i, 1, 15) {
            ...
            string right = make_right(left);
            if (i + LEN(right) == length) {
                chmin(ans, left + right);
                continue;
            }
            else if (i + LEN(right) > length) {
                break;
            }
            while (LEN(right) > 0) {
                left += right[0];
                ...
                while (now > 0) {
                    ...
                    if (left[now - 1] == left[now]) {
                        break;
                    }
                    swap(left[now - 1], left[now]);
                    now--;
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 405_DIV2_1000.sample");
