...
struct FindingSquareInTable {
    ...
    // sqrt(x)の整数解
    // なければ-1
    long long square_root(const long long x) {
        if (x < 0) {
            return -1;
        }
        long long a = (long long)sqrt(x);
        if (a * a == x) {
            return a;
        }
        if((a - 1) * (a - 1) == x) {
...
// CUT begin
ifstream data("../SRM 434_DIV1_250.sample");
