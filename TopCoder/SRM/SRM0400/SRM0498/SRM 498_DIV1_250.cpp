...
struct FoxSequence {
    ...
    string isValid(vector<int> _seq) {
        seq = _seq;

        int N = LEN(seq);
        FOR(a, 1, N) {
            int diff = seq[1] - seq[0];
            if (diff <= 0 or not ok(0, a, diff)) {
                continue;
            }

            FOR(b, a + 1, N) {
                diff = seq[a + 1] - seq[a];
                if (diff >= 0 or not ok(a, b, diff)) {
                    continue;
                }
                ...
                FOR(c, b, N) {
                    ...
                    FOR(d, c + 1, N - 1) {
                        diff = seq[c + 1] - seq[c];
                        if (diff <= 0 or not ok(c, d, diff)) {
                            continue;
                        }

                        diff = seq[d + 1] - seq[d];
                        if (diff < 0 and ok(d, N - 1, diff)) {
                            return "YES";
                        }
                    }
                }
...
// CUT begin
ifstream data("../SRM 498_DIV1_250.sample");
