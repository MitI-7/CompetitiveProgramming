...
struct PalindromeGame {
    vector<string> front;
    vector<int> back;
    int getMaximum(vector<string> _front, vector<int> _back) {
        front = _front, back = _back;

        map<string, vector<LL>> m;
        FOR(i, 0, LEN(front)) {
            m[front[i]].emplace_back(back[i]);
        }
        ...
        FOE(p, m) {
            ...
            if (s == t) {
                ...
                if (LEN(m[s]) % 2 == 1) {
                    ans -= m[s].back();
                    chmax(extra, m[s].back());
                }
            }
            else {
                FOR(i, 0, min(LEN(m[s]), LEN(m[t]))) {
                    ans += m[s][i] + m[t][i];
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 499_DIV2_950.sample");
...
bool do_test(vector<string> front, vector<int> back, int __expected) {
    time_t startClock = clock();
    PalindromeGame *instance = new PalindromeGame();
    int __result = instance->getMaximum(front, back);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(front, back, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1589606858;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "PalindromeGame (950 Points)" << endl << endl;
    }
