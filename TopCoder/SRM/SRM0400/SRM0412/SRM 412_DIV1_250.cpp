...
struct ForbiddenStrings {
    int n;
    long long countNotForbidden(int _n) {
        n = _n;

        auto dp = make_v<LL>(n + 10, 4, 4);
        dp[0][3][3] = 1;
        ...
        FOR(i, 0, n) {
            FOR(p1, 0, 4) {
                FOR(p2, 0, 4) {
                    FOR(c, 0, 3) {
                        if (p1 == 3 or p2 == 3 or (p1 == p2 or p1 == c or p2 == c)) {
                            dp[i + 1][c][p1] += dp[i][p1][p2];
                        }
                    }
                }
            }
...
// CUT begin
ifstream data("../SRM 412_DIV1_250.sample");
