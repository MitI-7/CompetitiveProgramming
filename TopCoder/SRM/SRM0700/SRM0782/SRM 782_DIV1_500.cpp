...
struct RankingStudents {
    ...
    vector<int> a;
    vector<int> b;
    string rankingPossible(int _n, vector<int> _f, vector<int> _a, vector<int> _b) {
        n = _n, f = _f, a = _a, b = _b;

        auto graph = make_v<int>(n, 0);

        auto in = make_v<int>(n);
        auto out = make_v<int>(n);
        FOR(i, 0, LEN(a)) {
...
// CUT begin
ifstream data("../SRM 782_DIV1_500.sample");
...
bool do_test(int n, vector<int> f, vector<int> a, vector<int> b, string __expected) {
    time_t startClock = clock();
    RankingStudents *instance = new RankingStudents();
    string __result = instance->rankingPossible(n, f, a, b);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(n, f, a, b, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1596791170;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "RankingStudents (500 Points)" << endl << endl;
    }
