...
// カッコ列から木を作成する
Tree make_tree_from_brackets(std::string brackets) {
    node = 1;
...
struct JustBrackets {
    ...
    bool can_make(const string &s, const string &t) {
        Tree t1 = make_tree_from_brackets("(" + s + ")");
        Tree t2 = make_tree_from_brackets("(" + t + ")");
        return is_subtree(t2, t1);
    }
...
// CUT begin
ifstream data("../SRM 731_DIV2_1000.sample");
