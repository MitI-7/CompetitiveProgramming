...
struct NewBanknote {
    ...
    int solve(const int p) {
        vector<int> coins = {1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000};
        reverse(ALL(coins));
    ...
    vector<int> fewestPieces(int _newBanknote, vector<int> _amountsToPay) {
        newBanknote = _newBanknote, amountsToPay = _amountsToPay;

        auto ans = make_v<int>(LEN(amountsToPay));
        FOR(i, 0, LEN(amountsToPay)) {
            ans[i] = solve(amountsToPay[i]);
        }
...
// CUT begin
ifstream data("../SRM 756_DIV1_250.sample");
