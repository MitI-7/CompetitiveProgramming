...
struct MazeWithKeys {
    ...
    bool ok(int sy, int sx) {
        queue<pair<int, int>> que;
        que.push(make_pair(sy, sx));

        auto used = make_v<bool>(H, W);
        auto have_key = make_v<bool>(256);
        map<char, vector<pair<int, int>>> keep;
        ...
        while (not que.empty()) {
            ...
            FOR(i, 0, 4) {
                ...
                if (inside(ny, nx, H, W)) {
                    // key
                    if ('a' <= a[ny][nx] and a[ny][nx] <= 'z') {
                        auto key = a[ny][nx];
                        have_key[key] = true;
                        // 手持ち
                        while (not keep[key].empty()) {
                            que.push(keep[key].front()); keep[key].pop_back();
                        }
                    }
                    // door
                    if ('A' <= a[ny][nx] and a[ny][nx] <= 'Z') {
                        ...
                        if (have_key[key]) {
                            ...
                            keep[key].emplace_back(make_pair(ny, nx));
                        }
                    } else if (a[ny][nx] != '#' and not used[ny][nx]) {
                        que.emplace(make_pair(ny, nx));
                    }
                }
            }
    ...
    int startingPoints(vector<string> _a) {
        a = _a;

        H = LEN(a);
        W = LEN(a[0]);

        auto ng = make_v<bool>(H, W);
        int gy = -1, gx = -1;
        FOR(y, 0, H) {
            FOR(x, 0, W) {
                if (a[y][x] == '*') {
        ...
        while (not que.empty()) {
            ...
            FOR(i, 0, 4) {
                int ny = dy4[i] + y;
                int nx = dx4[i] + x;
                if (inside(ny, nx, H, W) and (a[ny][nx] == '.' or ('a' <= a[ny][nx] and a[ny][nx] <= 'z')) and not ng[ny][nx]) {
                    que.emplace(make_pair(ny, nx));
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 736_DIV2_1000.sample");
