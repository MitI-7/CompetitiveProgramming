...
struct SimpleMazeEasy {

    map<int, int> calc(int sy, int sx, const vector<vector<int>> &f) {
        vector<vector<int>> memo(f.size(), vector<int>(f[0].size(), INF));
        memo[sy][sx] = 0;
        ...
        while (not que.empty()) {
            ...
            FOR(i, 0, 4) {
                int ny = dy4[i] + y;
                int nx = dx4[i] + x;
                if (inside(ny, nx, f.size(), f[0].size()) and f[ny][nx] == 1) {
                    if (memo[ny][nx] == INF) {
    ...
    LL sample(int n) {
        int h= n * 3;
        int w = n * 3;
        auto f = make_v<int>(n * 3, n * 3);
        FOR(y, 0, h) {
            FOR(x, 0, w) {
                if ((n <= y and y < 2 * n) or (n <= x and x < 2 * n)) {
    ...
    int findSum(long long n) {
        LL MOD = 1000000000 + 7;
        LL a = (333333332ll * powmod(n, 3, MOD)) % MOD;
        LL b = (666666691ll * powmod(n, 5, MOD)) % MOD;
        return (a + b) % MOD;
    }
};

// CUT begin
ifstream data("../SRM 723_DIV2_1000.sample");
