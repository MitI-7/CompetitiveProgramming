...
struct Coastlines {
    ...
    int dfs(int y, int x, vector<vector<bool>> &used) {
        used[y][x] = true;
        ...
        FOR(i, 0, 4) {
            ...
            if (inside(ny, nx, H, W) and not used[ny][nx] and map[ny][nx] == 'X') {
                ans += dfs(ny, nx, used);
            }
        }
    ...
    int longest(vector<string> _map) {
        map = _map;
        ...
        FOR(y, 0, H) {
            FOR(x, 0, W) {
                if (map[y][x] == 'X' and not used[y][x]) {
                    chmax(ans, dfs(y, x, used));
                }
            }
        }
...
// CUT begin
ifstream data("../Combined SRM 764_DIV1_275.sample");
...
bool do_test(vector<string> map, int __expected) {
    ...
    Coastlines *instance = new Coastlines();
    int __result = instance->longest(map);
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "Coastlines (275 Points)" << endl << endl;
    }
