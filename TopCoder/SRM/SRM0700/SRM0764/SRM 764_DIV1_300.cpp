...
public:
    Point<T> a, b, c, d;    // a -> b -> c -> d -> aと辺を張る
    Rectangle() {}
    Rectangle(Point<T> &a, Point<T> &b, Point<T> &c, Point<T> d) : a(a), b(b), c(c), d(d) {}

    bool is_rectangle() {
        if(Point<T>::dot(b - a, c - b) == 0 and Point<T>::dot(c - b, d - c) == 0 and Point<T>::dot(d - c, a - d) == 0 and Point<T>::dot(a - d, b - a) == 0) {
            return true;
        }
        return false;
    }
    ...
    double area() {
        return abs(Point<T>::cross(b - a, d - a));
    }
...
struct RectangleHunt {
    ...
    double largest(vector<int> _x, vector<int> _y) {
        ...
        auto points = make_v<Point<double>>(N);
        FOR(i, 0, N) {
            points[i] = Point<double>(y[i], x[i]);
        }
            ...
            FOR(b, 0, N) {
                FOR(c, 0, N) {
                    FOR(d, 0, N) {
                        ...
                        Rectangle<double> r(points[a], points[b], points[c], points[d]);
                        if (r.is_rectangle()) {
                            chmax(ans, r.area());
                        }
                    }
...
// CUT begin
ifstream data("../Combined SRM 764_DIV1_300.sample");
...
bool do_test(vector<int> x, vector<int> y, double __expected) {
    ...
    RectangleHunt *instance = new RectangleHunt();
    double __result = instance->largest(x, y);
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "RectangleHunt (300 Points)" << endl << endl;
    }
