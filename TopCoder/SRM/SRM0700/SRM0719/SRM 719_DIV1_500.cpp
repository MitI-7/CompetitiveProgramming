...
public:
    ...
    void init(int _num_of_node) {
        ...
        this->weight.resize(_num_of_node);
    }
...
pair<LL, LL> dfs(int u, Tree &tree) {
    LL use = tree.weight[u];
    LL not_use = 0;
    FOE(v, tree.children[u]) {
        auto p = dfs(v, tree);
        use += p.first;
        not_use += max(p.first, p.second);
    }
    use = max(0LL, use);
    not_use = max(0LL, not_use);
    return make_pair(use, not_use);
}

struct OwaskiAndTree {
    vector<int> parent;
    vector<int> pleasure;
    int maximalScore(vector<int> _parent, vector<int> _pleasure) {
        parent = _parent, pleasure = _pleasure;
...
// CUT begin
ifstream data("../SRM 719_DIV1_500.sample");
