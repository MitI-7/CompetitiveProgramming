...
struct LongMansionDiv1 {
    ...
    int eX;
    int eY;
    long long minimalTime(vector<int> _t, int _sX, int _sY, int _eX, int _eY) {
        t = _t, sX = _sX, sY = _sY, eX = _eX, eY = _eY;
        ...
        FOR(x, 0, H) {
            LL tmp = 0;
            FOR(j, min(x, sX), max(x, sX) + 1) {
                tmp += t[j];
            }
            FOR(j, min(x, eX), max(x, eX) + 1) {
                tmp += t[j];
            }
            tmp += (w + 1) * t[x];
            tmp -= 2 * t[x];
...
// CUT begin
ifstream data("../SRM 719_DIV1_250.sample");
