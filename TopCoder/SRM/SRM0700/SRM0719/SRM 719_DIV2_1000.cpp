...
void dfs(int u, int now, vector<int> &x, Tree &tree, vector<vector<int>> &weight) {
    x[u] = now;
    FOE(v, tree.children[u]) {
        dfs(v, now ^ weight[u][v], x, tree, weight);
    }
}

template <typename T> T max_xor(const std::vector<T> &arr) {
...
struct TwoDogsOnATree {
    vector<int> parent;
    vector<int> w;

    int maximalXorSum(vector<int> _parent, vector<int> _w) {
        parent = _parent, w = _w;
        ...
        auto weight = make_v<int>(N, N);

        FOR(i, 0, N - 1) {
            tree.add_undirected_edge(i + 1, parent[i]);
            weight[i + 1][parent[i]] = w[i];
            weight[parent[i]][i + 1] = w[i];
        }
        tree.build_directed_tree(0);

        auto x = make_v<int>(N);
        dfs(0, 0, x, tree, weight);

        vector<int> xor_list;
        xor_list.emplace_back(0);
        FOR(u, 0, N) {
            FOR(v, u + 1, N) {
                xor_list.emplace_back(x[u] ^ x[v]);
            }
        }

        return max_xor(xor_list);
    }
};

// CUT begin
ifstream data("../SRM 719_DIV2_1000.sample");
