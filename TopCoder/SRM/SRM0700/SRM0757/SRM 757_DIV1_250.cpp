...
struct CentipedeSocks {
    ...
    LL func(int last, const vector<LL> &v) {
        const int N = LEN(v);
        auto dp = make_v<LL>(LEN(v) + 1, C);
        fill_v(dp, -1);
        dp[0][0] = 0;
        ...
        FOR(i, 0, N) {
            FOR(j, 0, C) {
                ...
                FOR(k, 0, C) {
                    ...
                    if (j + k < C) {
                        LL ext = k * F + min<LL>(v[i] - k * F, F - 1);
                        chmax(dp[i + 1][j + k], dp[i][j] + ext);
                    }
                }
            }
        }
        ...
        LL ans = 0;
        FOR(i, 1, C + 1) {
            if (C - i >= 0 and i * F <= v[last]) {
                chmax(ans, dp[N][C - i] + i * F);
            }
        }
        return ans;
    }

    int fewestSocks(int _C, int _F, vector<int> _sockCounts) {
        C = _C, F = _F, sockCounts = _sockCounts;

        LL base = 0;
        auto v = make_v<LL>(0);
        int num = 0;
        FOR(i, 0, LEN(sockCounts)) {
            if (sockCounts[i] < F) {
                base += sockCounts[i];
            }
            else {
                v.emplace_back(sockCounts[i]);
                num += sockCounts[i] / F;
            }
        }
...
// CUT begin
ifstream data("../SRM 757_DIV1_250.sample");
