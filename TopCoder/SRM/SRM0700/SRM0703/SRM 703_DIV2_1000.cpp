...
struct TreeDiameters {
    vector<int> p;
    vector<vector<int>> tree;

    int dfs(int u, int parent, int len, map<int, int> &m) {
        m[len] += 1;

        int maxi = len;
        FOE(v, tree[u]) {
            if (v != parent) {
                chmax(maxi, dfs(v, u, len + 1, m));
            }
        }
        return maxi;
    }

    int getMax(vector<int> _p) {
        p = _p;

        tree = make_v<int>(1100, 0);
        FOR(i, 0, LEN(p)) {
            tree[i + 1].emplace_back(p[i]);
            tree[p[i]].emplace_back(i + 1);
        }

        int ans = 1;
        FOR(center, 0, LEN(tree)) {

            int maxi_len = 0;
            auto memo = make_v<map<int, int>>(0);
            FOE(v, tree[center]) {
                map<int, int> m;
                chmax(maxi_len, dfs(v, center, 1, m));
                memo.emplace_back(m);
            }

            FOR(len, 1, maxi_len + 1) {
                int tmp = 0;
                FOR(i, 0, LEN(memo)) {
                    FOR(j, i + 1, LEN(memo)) {
                        tmp += memo[i][len] * memo[j][len];
                    }
                }
...
// CUT begin
ifstream data("../SRM 703_DIV2_1000.sample");
