...
struct Unpacking {
    ...
    int func(vector<int> &v) {
        int N = LEN(v);
        auto dp = make_v<int>(N + 1, 10000 * 50 + 100);
        fill_v(dp, INF);
        dp[0][0] = 0;
        ...
        FOR(i, 0, N) {
            FOR(j, 0, LEN(dp[0])) {
                ...
                chmin(dp[i + 1][j + v[i] - 1], dp[i][j] + cost[i]);
    ...
    int func2() {
        const int N = LEN(a);
        auto dp = make_v<int>(N + 1, 20000 * 50 + 100);
        fill_v(dp, INF);
        dp[0][0] = 0;
        ...
        FOR(i, 0, N) {
            FOR(j, 0, LEN(dp[0])) {
                ...
                chmin(dp[i + 1][j + a[i] + b[i]], dp[i][j] + cost[i]);
                chmin(dp[i + 1][j], dp[i][j]);
            }
        }

        int ans = INF;
        FOR(j, 2 * K - 1, LEN(dp[0])) {
            chmin(ans, dp[N][j]);
        }
    ...
    int getcost(vector<int> _a, vector<int> _b, vector<int> _cost, int _K) {
        a = _a, b = _b, cost = _cost, K = _K;

        int ans = min(min(func(a), func(b)), func2());
        if (ans == INF) {
            return -1;
        }
        return ans;
...
// CUT begin
ifstream data("../SRM 726_DIV1_250.sample");
