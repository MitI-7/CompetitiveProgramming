...
struct OneHandSort {
    vector<int> target;
    vector<int> sortShelf(vector<int> _target) {
        target = _target;
        ...
        FOR(i, 0, N) {
            ...
            ans.emplace_back(i);
            ans.emplace_back(target_idx);
            ans.emplace_back(N);

            swap(target[target_idx], target[i]);
        }
...
// CUT begin
ifstream data("../SRM 755_DIV1_250.sample");
