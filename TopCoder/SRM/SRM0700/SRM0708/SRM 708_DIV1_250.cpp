...
struct BalancedStrings {

    int instability(const vector<string> &v) {
        int ans = 0;
        FOE(s, v) {
            FOR(i, 0, LEN(s) - 1) {
                ans += (s[i] != s[i + 1]);
    ...
    int similarity(const vector<string> &v) {
        int ans = 0;
        FOR(i, 0, LEN(v)) {
            FOR(j, i + 1, LEN(v)) {
    ...
    bool check(vector<string> &v) {
        FOR(i, 0, LEN(v)) {
            if (LEN(v[i]) == 0 or LEN(v[i]) > 100) {
                return false;
            }
            FOR(j, 0, LEN(v[i])) {
                if (v[i][j] < 'a' or 'z' < v[i][j]) {
                    return false;
                }
            }
        }

        return instability(v) == similarity(v);
    }
    ...
    vector<string> solve(int N) {
        ...
        int rest = similarity(ans);

        FOR(i, 0, N) {
            for (int j = 0; j < 99 and rest > 0; ++j) {
                if (j % 2 == 0) {
                    ans[i] += ('k' + 2 * i + 1);
                }
                else {
...
// CUT begin
ifstream data("../SRM 708_DIV1_250.sample");
