...
struct MostFrequentLastDigit {
    ...
    bool is_valid(int n, int d, vector<int> &v) {
        assert(LEN(v) == n);

        map<int, int> count;
        FOR(i, 0, n) {
            assert(0 <= v[i] and v[i] <= 1000000000);
            assert(v[i] % 10 != 0);
        ...
        FOE(p, count) {
            if (p.first != d and p.second >= count[d]) {
                return false;
            }
        }
        return true;
...
// CUT begin
ifstream data("../SRM 747_DIV1_250.sample");
