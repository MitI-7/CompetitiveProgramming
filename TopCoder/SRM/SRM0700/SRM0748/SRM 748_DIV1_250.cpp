...
struct EraseToGCD {
    vector<int> S;
    int goal;
    int countWays(vector<int> _S, int _goal) {
        S = _S, goal = _goal;
        const auto N = LEN(S);
        sort(ALL(S));

        auto dp = make_v<LL>(N + 10, 1100);
...
// CUT begin
ifstream data("../SRM 748_DIV1_250.sample");
