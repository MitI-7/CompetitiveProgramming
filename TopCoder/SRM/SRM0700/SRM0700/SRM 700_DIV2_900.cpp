...
struct XYZCoder {
    ...
    int countWays(int _room, int _size) {
        room = _room, size = _size;
        int n = room * size;
        auto dp = make_v<mint<MOD>>(room + 100, n + 100);
        dp[0][n] = 1;
        FOR(i, 0, room) {
            for(int j = room * size; j >= 0; --j) {
                if (j - 1 >= 0 and size * i <= n - j) {
                    dp[i][j - 1] += dp[i][j];
                    dp[i + 1][j - 1] += dp[i][j];
                }
            }
...
// CUT begin
ifstream data("../SRM 700_DIV2_900.sample");
