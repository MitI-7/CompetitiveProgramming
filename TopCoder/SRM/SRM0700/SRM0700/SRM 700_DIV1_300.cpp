...
struct FindingFriend {
    ...
    bool ok(int room_idx, int roomSize, vector<int> leaders, int friendPlace) {
        const LL N = LEN(leaders);

        if (leaders[room_idx] > friendPlace) {
            return false;
        }

        // room iに[begin, last)を割り当てる
        LL last = N * roomSize;
        for (int i = N - 1; i >= 0; --i) {
            LL begin = (last - roomSize) + 1;

            if (i == room_idx) {
                if (not(begin <= friendPlace and friendPlace < last)) {
                    begin++;
                }
            } else {
                if (begin <= friendPlace and friendPlace < last) {
                    begin--;
                }
            }

            for (int j = N - 1; j >= 0; --j) {
                if (begin <= leaders[j] and leaders[j] < last) {
                    begin--;

                    if (i != room_idx and begin == friendPlace) {
                        begin--;
                    }
                }
            }
    ...
    int find(int roomSize, vector<int> leaders, int friendPlace) {
        ...
        int ans = 0;
        FOR(i, 0, LEN(leaders)) {
            ans += ok(i, roomSize, leaders, friendPlace);
        }
...
// CUT begin
ifstream data("../SRM 700_DIV1_300.sample");
