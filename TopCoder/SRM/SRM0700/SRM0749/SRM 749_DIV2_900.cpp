...
// 累積積
// 要素に0があるとだめ
template<typename T>class CumulativeProduct {
public:
    const long long mod;
    std::vector<T> memo1;               // 1次元用の累積積表
    std::vector<T> inv;

    CumulativeProduct(long long mod) : mod(mod) {

    }

    void build(const std::vector<T> &line) {
        this->memo1.assign(line.size() + 1, 1);
        this->inv.assign(line.size() + 1, 1);

        memo1[0] = 1;
        for (int i = 0; i < line.size(); ++i) {
            assert(line[i] != 0);
            (memo1[i + 1] = line[i] * memo1[i]) %= this->mod;
        }

        for (int i = 0; i < line.size(); ++i) {
            inv[i] = modinv(memo1[i], this->mod);
        }
    }

    // [left, right)の積
    T product(int left, int right) {
        return memo1[right] * inv[left];
    }
};

struct CountSubarrays {
    LL MOD = 744444499;
    ...
    LL func(vector<LL> &A, LL X) {
        ...
        auto inv = make_v<LL>(LEN(dp));
        FOR(i, 0, LEN(dp)) {
            inv[i] = modinv(dp[i], MOD);
        }
    ...
    LL solve(vector<LL> &A, LL X) {
        ...
        FOR(i, 0, LEN(A)) {
            if (A[i] == 0) {
                if (not v.empty()) {
                    ans += func(v, X);
                    count += LEN(v) + ((LL)LEN(v) * (LEN(v) - 1)) / 2;
                    v = make_v<LL>(0);
                }
            }
            else {
        ...
        if (X == 0) {
            ans += LEN(A) + ((LL)LEN(A) * (LEN(A) - 1) / 2) - count;
        }

        return ans;
    }

    long long count(vector<int> _Aprefix, int _n, int _a, int _b, int _m, int _X) {
        auto Aprefix = _Aprefix;
        LL N = _n;
        LL a = _a;
        LL b = _b;
        LL m = _m;
...
// CUT begin
ifstream data("../SRM 749_DIV2_900.sample");
...
bool do_test(vector<int> Aprefix, int n, int a, int b, int m, int X, long long __expected) {
    time_t startClock = clock();
    CountSubarrays *instance = new CountSubarrays();
    long long __result = instance->count(Aprefix, n, a, b, m, X);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if (do_test(Aprefix, n, a, b, m, X, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1586836858;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "CountSubarrays (900 Points)" << endl << endl;
    }
