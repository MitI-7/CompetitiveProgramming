...
struct SumProduct {
    ...
    int findSum(vector<int> _amount, int _blank1, int _blank2) {
        amount = _amount, blank1 = _blank1, blank2 = _blank2;

        vector<vector<LL>> comb(210, vector<LL>(210, 0));
        for(int i=0; i<210; i++) {
            comb[i][0] = 1;
            for(int j=1; j<=i; j++) {
        ...
        FOR(x, 0, 10) {
            FOR(y, 0, 10) {
                ...
                LL a = 0, b = 0;
                FOR(i, 0, blank1) {
                    (a += (LL)x * powmod(10, i, MOD)) %= MOD;
                }
                FOR(i, 0, blank2) {
                    (b += (LL)y * powmod(10, i, MOD)) %= MOD;
                }
                ...
                // dp[i][j] = 数値iまでみたとき，j個うめるような場合の数
                auto dp = make_v<int>(11, rest + 1);
                dp[0][0] = 1;
                FOR(i, 0, 10) {
                    FOR(j, 0, rest + 1) {
                        FOR(k, 0, amount[i] + 1) {
                            if (j + k <= rest) {
                                (dp[i + 1][j + k] += (dp[i][j] * comb[rest - j][k] % MOD)) %= MOD;
                            }
                        }
                    }
                }
                ...
                (ans += (((a * b) % MOD) * dp[10][rest]) % MOD) %= MOD;
            }
        }
...
// CUT begin
ifstream data("../SRM 720_DIV1_250.sample");
