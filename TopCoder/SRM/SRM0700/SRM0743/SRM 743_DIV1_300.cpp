...
struct MaximizingGCD {
    vector<int> A;
    int maximumGCDPairing(vector<int> _A) {
        A = _A;
        ...
        FOR(i, 0, N) {
            FOR(j, i + 1, N) {
                ...
                FOE(d, dl) {
                    ...
                    bool ok = true;
                    FOE(p, m) {
                        if (p.first == 0 and p.second % 2 != 0) {
                            ok = false;
                        }
                        if (p.first != 0 and m[d - p.first] != p.second) {
                            ok = false;
                        }
                    }
                    if (ok) {
...
// CUT begin
ifstream data("../SRM 743_DIV1_300.sample");
