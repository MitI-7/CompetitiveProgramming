...
struct TreeDistanceConstruction {
    ...
    vector<int> construct(vector<int> _d) {
        ...
        int center_dist = (maxi + 1) / 2;
        int center_node1, center_node2;
        if (maxi % 2 == 1) {
            if (m[center_dist].empty()) {return {};}
            center_node1 = m[center_dist].back(); m[center_dist].pop_back();

            if (m[center_dist].empty()) {return {};}
            center_node2 = m[center_dist].back(); m[center_dist].pop_back();

            ans.emplace_back(center_node1);
            ans.emplace_back(center_node2);
        } else {
            if (m[center_dist].empty()) {return {};}
            center_node1 = center_node2 = m[center_dist].back(); m[center_dist].pop_back();
        }

        {
            int pre = center_node1;
            FOR(dist, center_dist + 1, maxi + 1) {
                if (m[dist].empty()) {return {};}

                ans.emplace_back(pre);
                ans.emplace_back(m[dist].back()); m[dist].pop_back();
                pre = ans.back();
            }
        }

        {
            int pre = center_node2;
            FOR(dist, center_dist + 1, maxi + 1) {
                if (m[dist].empty()) {return {};}

                while (not m[dist].empty()) {
                    ans.emplace_back(pre);
                    ans.emplace_back(m[dist].back()); m[dist].pop_back();
                }
                pre = ans.back();
...
// CUT begin
ifstream data("../SRM 704_DIV1_300.sample");
