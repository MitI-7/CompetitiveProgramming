...
struct MazeConstruct {
    ...
    int check(vector<vector<int>> &board) {
        auto memo = make_v<int>(board.size(), board[0].size());
        fill_v(memo, INF);
        queue<tuple<int, int, int>> que;
        que.emplace(make_tuple(0, 0, 0));
        while (not que.empty()) {
            int c, y, x;
            tie(c, y, x) = que.front(); que.pop();
            if (memo[y][x] < c) {
                continue;
            }
            memo[y][x] = c;
            FOR(i, 0, 4) {
                auto ny = dy4[i] + y;
                auto nx = dx4[i] + x;
                if (inside(ny, nx, board.size(), board[0].size())) {
                    if (board[ny][nx] == 0 and memo[ny][nx] > c + 1) {
                        memo[ny][nx] = c + 1;
                        que.emplace(make_tuple(c + 1, ny, nx));
                    }
                }
            }
        }

        return memo[board.size() - 1][board[0].size() - 1];
    }

    vector<vector<int>> solve(int k) {
        vector<vector<int>> field;
        if (k <= 49) {
            field = make_v<int>(1, k + 1);
        }
        else if (k <= 98) {
            field = make_v<int>(50, k - 48);
        }
        else {
            if (k % 2 == 1) {
                field = make_v<int>(50, 49);
            }
            else {
                field = make_v<int>(50, 50);
            }

            int m = 0;
            for (int y = 1; y < LEN(field); y += 2) {
                if (m % 2 == 0) {
                    FOR(x, 0, LEN(field[0]) - 1) {
                        field[y][x] = 1;
                        if (check(field) >= k) {
                            break;
                        }
...
// CUT begin
ifstream data("../SRM 707_DIV1_250.sample");
