...
struct GridSortMax {
    ...
    void swap_y(int y1, int y2, vector<vector<int>> &grid) {
        swap(grid[y1], grid[y2]);
    }

    void swap_x(int x1, int x2, vector<vector<int>> &grid) {
        FOR(y, 0, LEN(grid)) {
            swap(grid[y][x1], grid[y][x2]);
        }
    }

    string findMax(int _n, int _m, vector<int> _grid) {
        n = _n, m = _m;

        auto grid = make_v<int>(n, m);
        FOR(y, 0, n) {
            FOR(x, 0, m) {
                grid[y][x] = _grid[y * m + x];
            }
        }

        auto used_y = make_v<bool>(n);
        auto used_x = make_v<bool>(m);
            ...
            FOR(x, 0, m) {
                ...
                FOR(y1, 0, n) {
                    FOR(x1, 0, m) {
                        if (grid[y1][x1] == t) {
                            if (y == y1 and x == x1) {
                                used_y[y] = used_x[x] = true;
                            }
                            else if (y == y1 and not used_x[x] and not used_x[x1]) {
                                swap_x(x, x1, grid);
                                used_y[y] = used_x[x] = true;
                            }
                            else if (x == x1 and not used_y[y] and not used_y[y1]) {
                                swap_y(y, y1, grid);
                                used_y[y] = used_x[x] = true;
                            }
                            else if (not used_x[x] and not used_x[x1] and not used_y[y] and not used_y[y1]) {
                                swap_x(x, x1, grid);
                                swap_y(y, y1, grid);
                                used_y[y] = used_x[x] = true;
                            }
...
// CUT begin
ifstream data("../SRM 702_DIV1_300.sample");
