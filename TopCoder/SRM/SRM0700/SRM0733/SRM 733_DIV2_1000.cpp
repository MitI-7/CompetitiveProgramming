...
struct HamiltonianPathsInGraph {
    int N;
    vector<string> X;

    vector<int> findPath(vector<string> _X) {
        X = _X;
        ...
        FOR(u, 0, N) {
            ...
            // ans.back() -> u
            else if (X[ans.back()][u] == '+') {
                ans.emplace_back(u);
            }
            else {
                FOR(i, 0, LEN(ans) - 1) {
                    // ans[i] -> u -> ans[i + 1]
                    if (X[ans[i]][u] == '+' and X[u][ans[i + 1]] == '+') {
...
// CUT begin
ifstream data("../SRM 733_DIV2_1000.sample");
...
bool do_test(vector<string> X, vector<int> __expected) {
    ...
    HamiltonianPathsInGraph *instance = new HamiltonianPathsInGraph();
    vector<int> __result = instance->findPath(X);
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "HamiltonianPathsInGraph (1000 Points)" << endl << endl;
    }
