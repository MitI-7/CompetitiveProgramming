...
struct MeanMedian {
    ...
    int minEffort(int _needMean, int _needMedian, vector<int> _d) {
        needMean = _needMean, needMedian = _needMedian, d = _d;

        N = LEN(d);
        sort(ALL(d));

        LL need_day = 0;
        LL need_total_grade = N * needMean;
        LL h = N / 2;
        FOR(i, 0, h + 1) {
            need_day += d[i] * needMedian;
            need_total_grade -= needMedian;
        }

        if (need_total_grade <= 0) {
            return need_day;
        }

        FOR(i, 0, N) {
            LL rest = min(need_total_grade, 10LL);
            if (i <= h) {
                rest = min(rest, (10 - needMedian));
            }

            need_day += d[i] * rest;
            need_total_grade -= rest;
        }
...
// CUT begin
ifstream data("../SRM 768_DIV1_250.sample");
