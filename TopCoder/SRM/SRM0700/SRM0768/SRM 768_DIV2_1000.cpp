...
struct MeanMedianCount {
    ...
    int needMean;
    int needMedian;
    int getCount(int _N, int _needMean, int _needMedian) {
        N = _N, needMean = _needMean, needMedian = _needMedian;

        auto dp = make_v<LL>(N + 1, N * 10 + 100, N + 1);
        dp[0][0][0] = 1;
        FOR(i, 0, N) {
            FOR(t, 0, N * 10 + 1) {
                FOR(c, 0, N) {
                    FOR(g, 0, 10 + 1) {
                        (dp[i + 1][t + g][c + (g >= needMedian)] += dp[i][t][c]) %= MOD;
                    }
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 768_DIV2_1000.sample");
...
bool do_test(int N, int needMean, int needMedian, int __expected) {
    time_t startClock = clock();
    MeanMedianCount *instance = new MeanMedianCount();
    int __result = instance->getCount(N, needMean, needMedian);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(N, needMean, needMedian, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1588133775;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "MeanMedianCount (1000 Points)" << endl << endl;
    }
