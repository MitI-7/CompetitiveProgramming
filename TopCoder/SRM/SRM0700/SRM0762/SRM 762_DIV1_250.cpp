...
struct LexicographicPartition {
    int n;
    vector<int> Aprefix;
    int seed;
    int Arange;
    vector<int> positiveSum(int _n, vector<int> _Aprefix, int _seed, int _Arange) {
        n = _n, Aprefix = _Aprefix, seed = _seed, Arange = _Arange;
        ...
        LL state = seed;
        FOR(i, LEN(Aprefix), n) {
            state = (1103515245LL * state + 12345);
            A[i] = state % (2LL * Arange + 1);
            A[i] = A[i] - Arange;
            state = state % (1ULL << 31ULL);
        }
        ...
        FOR(i, 0, n) {
            ...
            if (now > 0 and (i == n - 1 or total > 0)) {
                B.emplace_back(length);
                length = 0;
                now = 0;
            }
        }
        ...
        ans.emplace_back(LEN(B));
        FOR(i, 0, min(200, LEN(B))) {
...
// CUT begin
ifstream data("../SRM 762_DIV1_250.sample");
