...
struct CircleCity {
    ...
    LL count(int start, LL x) {
        LL count = 1;
        LL now = 0;
        LL c = 0;
        FOR(i, 0, LEN(dist) - 1) {
            int idx = (i + start) % LEN(dist);
            c += dist[idx];
            if (c - now > x) {
                now = c;
                count++;
    ...
    bool ok(LL x) {
        LL need = INF;
        FOR(i, 0, LEN(dist)) {
            chmin(need, count(i, x));
        }
        return need <= k;
    }

    long long binary_search_left(long long low, long long high) {
        // (low, high)
        while (high - low > 1) {
    ...
    int findMin(vector<int> _dist, int _k) {
        dist = _dist, k = _k;

        return binary_search_left(-1, INF);
    }
};

// CUT begin
ifstream data("../SRM 718_DIV1_500.sample");
