...
struct EllysThreePrimes {
    ...
    vector<int> getPrimes(vector<int> _sums) {
        sums = _sums;
        auto is_prime = make_prime_list(100100);
        auto prime_list = make_v<int>(0);
        FOR(i, 10000, 100000) {
            if (is_prime[i]) {
        ...
        const vector<int> tmp = {1, 10, 100, 1000, 10000};
        vector<int> v(5);

        vector<int> p1_v(5);
        FOR(i, 0, LEN(prime_list)) {
            int p1 = prime_list[i];
            FOR(d, 0, 5) {
                p1_v[d] = (p1 / tmp[d]) % 10;
            }

            FOR(j, i + 1, LEN(prime_list)) {
                int p2 = prime_list[j];

                bool ok = true;
                FOR(d, 0, 5) {
                    v[d] = sums[d] - p1_v[d] - ((p2 / tmp[d]) % 10);
                    if (v[d] < 0 or v[d] >= 10) {
                        ok = false;
                        break;
                    }
                ...
                if (10000 <= value and value < 100000 and is_prime[value]) {
...
// CUT begin
ifstream data("../SRM 759_DIV1_250.sample");
