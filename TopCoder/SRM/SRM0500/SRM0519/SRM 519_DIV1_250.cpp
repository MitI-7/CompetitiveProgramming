...
struct BinaryCards {
    long long A;
    long long B;
    long long largestNumber(long long _A, long long _B) {
        A = _A, B = _B;

        LL ans = 0;
        for (int b = 63; b >= 0; --b) {
            if (is_bit_on(B, b) == is_bit_on(A, b)) {
                ans = bit_set(ans, b, is_bit_on(A, b));
            }
            else {
                for (int i = b; i >= 0; --i) {
                    ans = bit_set(ans, i, 1);
...
// CUT begin
ifstream data("../SRM 519_DIV1_250.sample");
