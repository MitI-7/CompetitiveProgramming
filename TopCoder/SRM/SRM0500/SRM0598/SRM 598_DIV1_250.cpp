...
struct BinPacking {
    ...
    int solve(int x, int num100, vector<int> v) {
        int ans = x;
        num100 -= (x * 3);
    ...
    int minBins(vector<int> _item) {
        ...
        FOR(i, 0, LEN(item)) {
            if (item[i] == 100) {
                ...
                v.emplace_back(item[i]);
            }
        }

        int best = INF;
        FOR(i, 0, (num100 / 3) + 1) {
            chmin(best, solve(i, num100, v));
        }
...
// CUT begin
ifstream data("../SRM 598_DIV1_250.sample");
