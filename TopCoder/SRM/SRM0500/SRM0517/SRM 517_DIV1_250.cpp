...
struct CompositeSmash {
    ...
    bool can_avoid(int x) {
        ...
        int num = 0;
        FOR(i, 0, LEN(v)) {
            if ((not can_avoid(v[i].first)) or (not can_avoid(v[i].second))) {
                num++;
            }
        }

        return memo[x] = (LEN(v) > num);
    }

    string thePossible(int _N, int _target) {
        N = _N, target = _target;
...
// CUT begin
ifstream data("../SRM 517_DIV1_250.sample");
