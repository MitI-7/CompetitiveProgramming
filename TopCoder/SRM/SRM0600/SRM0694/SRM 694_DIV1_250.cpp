...
struct TrySail {
    vector<int> strength;
    int get(vector<int> _strength) {
        strength = _strength;
        const auto N = LEN(strength);

        auto dp = make_v<bool>(2, 256, 256, 256);
        dp[0][0][0][0] = true;

        FOR(i, 0,  N) {
            auto value = strength[i];

            FOR(a, 0, 256) {
                FOR(b, 0, 256) {
                    FOR(c, 0, 256) {
                        if (dp[i % 2][a][b][c]) {
                            dp[(i + 1) % 2][a ^ value][b][c] = true;
                            dp[(i + 1) % 2][a][b ^ value][c] = true;
                            dp[(i + 1) % 2][a][b][c ^ value] = true;
                        }
                        dp[i % 2][a][b][c] = false;
                    }
                }
...
// CUT begin
ifstream data("../SRM 694_DIV1_250.sample");
