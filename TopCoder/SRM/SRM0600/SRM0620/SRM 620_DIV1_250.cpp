...
struct PairGame {
    ...
    set<pair<int, int>> gen(int x, int y) {
        ...
        while (true) {
            ...
            if (x == y) {
                candidate.insert(make_pair(x, 0));
                candidate.insert(make_pair(0, y));
                break;
            }
    ...
    int maxSum(int _a, int _b, int _c, int _d) {
        ...
        set<pair<int, int>> cand1 = gen(a, b);
        set<pair<int, int>> cand2 = gen(c, d);

        int ans = -1;
        FOE(p, cand1) {
            if (cand2.find(p) != cand2.end()) {
                chmax(ans, p.first + p.second);
            }
        }
...
// CUT begin
ifstream data("../SRM 620_DIV1_250.sample");
