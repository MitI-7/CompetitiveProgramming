...
struct RandomGraph {
    int n;
    int p;
    double probability(int _n, int _p) {
        n = _n, p = _p;
        double prob = p / 1000.0;

        auto dp = make_v<double>(55, 55, 55, 55);
        dp[0][0][0][0] = 1.0;
            ...
            FOR(a, 0, 51) {
                FOR(b, 0, 51) {
                    FOR(c, 0, 51) {
                        ...
                        {
                            // どれにもくっつかない
                            dp[i + 1][a + 1][b][c] += dp[i][a][b][c] * pow((1.0 - prob), i);
                        }
                        ...
                        // edge 1
                        {
                            // 1にひとつだけくっつき，2になる
                            if (a - 1 >= 0 and i - 1 >= 0) {
                                dp[i + 1][a - 1][b + 1][c] += dp[i][a][b][c] * prob * pow((1.0 - prob), i - 1) * a;
                            }

                            // 2にひとつだけくっつき，3になる
                            if (b - 1 >= 0 and i - 1 >= 0) {
                                dp[i + 1][a][b - 1][c + 1] += dp[i][a][b][c] * prob * pow((1.0 - prob), i - 1) * b * 2;
                            }
                        }
                        ...
                        // edge 2
                        {
                            // 1と1にくっつき，3になる
                            if (a - 2 >= 0 and i - 2 >= 0) {
                                dp[i + 1][a - 2][b][c + 1] += dp[i][a][b][c] * prob * prob * pow((1.0 - prob), i - 2) * (a * (a - 1) / 2.0);
                            }

                            // 2と同じコンポーネントの2にくっつき，3になる
                            if (b - 1 >= 0) {
                                dp[i + 1][a][b - 1][c + 1] += dp[i][a][b][c] * prob * prob * pow((1.0 - prob), i - 2) * b;
                            }
                        }
                    }
                }
...
// CUT begin
ifstream data("../SRM 620_DIV2_1000.sample");
