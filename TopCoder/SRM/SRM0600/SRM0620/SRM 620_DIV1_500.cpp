...
struct CandidatesSelection {
    ...
    string possible(vector<string> _score, vector<int> _result) {
        score = _score, result = _result;

        auto conditions = make_v<pair<int, int>>(0);
        FOR(i, 0, LEN(result) - 1) {
            conditions.push_back({result[i], result[i + 1]});
        }

        bool change = true;
        auto used = make_v<bool>(LEN(score[0]));
        while (change) {
            change = false;

            FOR(skill, 0, LEN(score[0])) {
                if (used[skill]) {
                    continue;
                }
                bool ok = true;
                FOE(p, conditions) {
                    ok &= score[p.first][skill] <= score[p.second][skill];
                }

                if (ok) {
                    used[skill] = true;
                    change = true;
                    auto tmp = make_v<pair<int, int>>(0);

                    FOE(p, conditions) {
                        if (score[p.first][skill] == score[p.second][skill]) {
...
// CUT begin
ifstream data("../SRM 620_DIV1_500.sample");
...
bool do_test(vector<string> score, vector<int> result, string __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(score, result, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1592811175;
