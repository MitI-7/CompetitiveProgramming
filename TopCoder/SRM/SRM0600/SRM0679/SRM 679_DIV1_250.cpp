...
struct FiringEmployees {
    ...
    LL dfs(int u, vector<vector<int>> &children) {

        LL ans = productivity[u] - salary[u];
    ...
    int fire(vector<int> _manager, vector<int> _salary, vector<int> _productivity) {
        manager = _manager, salary = _salary, productivity = _productivity;
...
// CUT begin
ifstream data("../SRM 679_DIV1_250.sample");
