...
struct ColorfulGardenHard {
    ...
    int count(string garden, string forbid) {
        ...
        unsigned int N = LEN(garden);
        map<int, int> count;
        FOR(i, 0, N) {
            count[garden[i]]++;
        }

        auto dp = make_v<mint<MOD>>(N + 1, (1ull << N) + 1, N + 1);
        dp[0][0][N] = 1;
        ...
        FOR(i, 0, N) {
            FOR(b, 0, (1ull << N)) {
                FOR(pre_idx, 0, N + 1) {
                    ...
                    FOR(now_idx, 0, N) {
                        if (forbid[i] == garden[now_idx]) {
                            continue;
                        }
                        if (is_bit_on(b, now_idx)) {
                            continue;
                        }
                        if (pre_idx != N and not is_bit_on(b, pre_idx)) {
                            continue;
                        }
                        if (pre_idx != N and garden[pre_idx] == garden[now_idx]) {
                            continue;
                        }

                        dp[i + 1][bit_set(b, now_idx, 1)][now_idx] += dp[i][b][pre_idx];
                    }
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 689_DIV2_1000.sample");
