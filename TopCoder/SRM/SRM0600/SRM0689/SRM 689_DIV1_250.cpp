...
struct ColorfulGarden {
    ...
    vector<string> rearrange(vector<string> _garden) {
        garden = _garden;
        auto N = LEN(garden[0]);
        ...
        auto v = make_v<pair<int, char>>(0);
        FOE(p, m) {
            v.emplace_back(make_pair(p.second, p.first));
            if (p.second > N) {
...
// CUT begin
ifstream data("../SRM 689_DIV1_250.sample");
