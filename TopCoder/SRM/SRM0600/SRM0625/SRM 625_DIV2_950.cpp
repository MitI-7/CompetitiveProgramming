...
struct ConundrumReloaded {
    ...
    int count(const string &s, const int first_type, const bool is_circle) {
        int ans = 0;
        int type = first_type;
        FOR(i, 0, LEN(s)) {
            assert(s[i] != '?');
    ...
    int minimumLiars(string _answers) {
        answers = _answers;

        const int N = LEN(answers);
        ...
        if (first_q == -1) {
            int ans = min(count(answers, 0, true), count(answers, 1, true));
            if (ans == INF) {
                return -1;
            }
        ...
        auto v = make_v<string>(0);
        string s;
        FOR(i, 1, N) {
            auto c = answers[(first_q + i) % N];
            if (c == '?') {
                if (not s.empty()) {
                    v.emplace_back(s);
                }
        ...
        int ans = 0;
        FOE(s, v) {
            ans += min(count(s, 0, false), count(s, 1, false));
        }
...
// CUT begin
ifstream data("../SRM 625_DIV2_950.sample");
