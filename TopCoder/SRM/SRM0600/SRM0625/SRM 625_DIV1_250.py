...
class PalindromePermutations:
    def palindromeProbability(self, word):
        ...
        for v in count.values():
            num_odd += (v % 2 == 1)
        if N % 2 == 0 and num_odd > 0:
            return 0.0
        if N % 2 == 1 and num_odd != 1:
            return 0.0

        a = 1.0 * math.factorial(N // 2)
        b = 1.0 * math.factorial(N)
        if N % 2 == 1:
            a = 1.0 * math.factorial((N - 1) // 2)
        for v in count.values():
            if v % 2 == 1:
                a /= math.factorial((v - 1) // 2)
            else:
                a /= math.factorial(v // 2)
...
def run_tests():
    ...
    with open("SRM 625_DIV1_250.sample", "r") as f:
        while True:
            label = f.readline()
