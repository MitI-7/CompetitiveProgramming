...
struct FoxesOfTheRoundTable {
    ...
    vector<pair<int, int>> solve(int d, const vector<pair<int, int>> &v) {

        auto used = make_v<bool>(N);
        used[0] = true;
        auto ans = make_v<pair<int, int>>(0);
        ans.emplace_back(v[0]);

        FOR(i, 1, N) {
            if (v[i].first - ans.back().first > d) {
                if (used[i - 1]) {
                    return {};
                }
        ...
        // check
        FOR(i, 0, N) {
            if (abs(ans[(i + 1) % N].first - ans[i].first) > d) {
                return {};
            }
        }
...
// CUT begin
ifstream data("../SRM 662_DIV1_250.sample");
