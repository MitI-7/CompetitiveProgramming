...
public:
    Point<T> a, b, c;    // a -> b -> c -> aと辺を張る
    Triangle() {}
    Triangle(Point<T> a, Point<T> b, Point<T> c) : a(a), b(b), c(c) {}
};

template <typename T> double sign(const Point<T> &p1, const Point<T> &p2, const Point<T> &p3) {
    return (p1.x - p3.x) * (p2.y - p3.y) - (p2.x - p3.x) * (p1.y - p3.y);
}

template <typename T> bool inside_triangle(const Point<T> &p, const Triangle<T> &triangle) {
    const auto d1 = sign(p, triangle.a, triangle.b);
    const auto d2 = sign(p, triangle.b, triangle.c);
    const auto d3 = sign(p, triangle.c, triangle.a);

    const bool has_neg = (d1 < 0) or (d2 < 0) or (d3 < 0);
    const bool has_pos = (d1 > 0) or (d2 > 0) or (d3 > 0);
...
struct Flee {
    vector<int> x;
    vector<int> y;
    double maximalSafetyLevel(vector<int> _x, vector<int> _y) {
        x = _x, y = _y;

        double ans = INF;
        FOR(i, 0, LEN(x)) {
            chmin(ans, euclidean_distance(0, 0, y[i], x[i]));
        }

        int overlap = 0;
        FOR(i, 0, LEN(x)) {
            FOR(j, i + 1, LEN(x)) {
                overlap += (x[i] == x[j] and y[i] == y[j]);
            }
        }
        if (LEN(x) - overlap <= 2) {
            return ans;
        ...
        Triangle<double> triangle(Point<double>(y[0], x[0]), Point<double>(y[1], x[1]), Point<double>(y[2], x[2]));
        if (not inside_triangle(Point<double>(0, 0), triangle)) {
            return ans;
        }
        ...
        FOR(i, 0, LEN(x)) {
            FOR(j, i + 1, LEN(x)) {
                auto d = euclidean_distance(y[i], x[i], y[j], x[j]);
                chmax(tmp, d / 2.0);
            }
        }
...
// CUT begin
ifstream data("../SRM 662_DIV2_1000.sample");
