...
struct GreaterGame {
    ...
    double calc(vector<int> _hand, vector<int> _sothe) {
        ...
        sort(ALL(hand));
        sort(RALL(sothe));
        while (not sothe.empty() and sothe.back() == -1) {
            sothe.pop_back();
        }
        ...
        auto used = make_v<bool>(N);
        {
            int i = 0;
            while (i < N) {
                if (not sothe.empty() and sothe.back() < hand[i]) {
                    sothe.pop_back();
                    used[i] = true;
                    ans++;
                }
...
// CUT begin
ifstream data("../SRM 637_DIV1_250.sample");
