...
struct ConnectingGameDiv2 {
    ...
    vector<bool> is_start, is_goal;
    map<int, int> occ;

    int func(const int s) {

        auto cost = make_v<int>(100);
        fill_v(cost, INF);
        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> que;
        que.emplace(make_pair(occ[s], s));
    ...
    int getmin(vector<string> _board) {

        board = _board;
        H = LEN(board);
        W = LEN(board[0]);

        graph = make_v<int>(100, 0);
        auto used = make_v<bool>(100, 100);
        is_start = make_v<bool>(100);
        is_goal = make_v<bool>(100);
        FOR(y, 0, H) {
            FOR(x, 0, W) {
                int c1 = board[y][x] - '0';
                occ[c1]++;
                ...
                FOR(i, 0, 8) {
                    ...
                    if (inside(ny, nx, H, W)) {
                        int c2 = board[ny][nx] - '0';
                        if (c1 != c2 and not used[c1][c2]) {
...
// CUT begin
ifstream data("../SRM 637_DIV2_1000.sample");
