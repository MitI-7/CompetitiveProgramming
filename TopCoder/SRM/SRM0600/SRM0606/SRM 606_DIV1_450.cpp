...
private:
    const long long NA = LONG_LONG_MIN; // 出現しない値
    unsigned long long counter = 0;
    long long candidate = NA;           // 過半数出現する値の候補
...
public:
    ...
    // cの出現数をカウント
    void count(const long long v) {
        this->num += (v == candidate);
        this->total++;
    }
...
struct EllysPairing {
    ...
    int getMax(int _M, vector<int> _count, vector<int> _first, vector<int> _mult, vector<int> _add) {
        M = _M, count = _count, first = _first, mult = _mult, add = _add;

        BoyerMooreMajorityVoteAlgorithm bm;

        FOR(i, 0, LEN(count)) {
            unsigned int now = first[i];
            bm.add(now);

            FOR(_, 0, count[i] - 1) {
                now = (now * mult[i] + add[i]) & (unsigned int)(M - 1);
                bm.add(now);
            }
        }
        ...
        if (bm.num * 2 >= bm.total) {
            return bm.total - bm.num;
        }
        else {
            return bm.total / 2;
        }
...
// CUT begin
ifstream data("../SRM 606_DIV1_450.sample");
