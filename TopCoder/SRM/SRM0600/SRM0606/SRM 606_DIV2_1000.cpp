...
struct EllysCandyGame {
    vector<int> sweets;
    map<pair<int, vector<bool>>, int> memo;

    int score_diff(int t, vector<bool> &used, const vector<int> &v) {
        bool ok = false;
        FOR(i, 0, LEN(v)) {
            ok |= (v[i] > 0 and not used[i]);
        }
        if (not ok) {
            return 0;
        }

        auto key = make_pair(t, used);
        ...
        if (t % 2 == 0) {
            int ans = -INF;
            FOR(i, 0, LEN(v)) {
                if (v[i] != 0 and not used[i]) {
                    int s = v[i];
                    if (i + 1 < LEN(used) and used[i + 1]) {
                        s *= 2;
                    }
                    if (0 <= i - 1 and used[i - 1]) {
                        s *= 2;
                    }
                    used[i] = true;
                    chmax(ans, score_diff(t + 1, used, v) + s);
                    used[i] = false;
                }
            }
            return memo[key] = ans;
            ...
            FOR(i, 0, LEN(v)) {
                if (v[i] != 0 and not used[i]) {
                    ...
                    used[i] = true;
                    chmin(ans, score_diff(t + 1, used, v) - s);
                    used[i] = false;
                }
            }
            return memo[key] = ans;
    ...
    string getWinner(vector<int> _sweets) {
        ...
        auto used = make_v<bool>(LEN(sweets));
        int diff = score_diff(0, used, sweets);
        if (diff == 0) {
            return "Draw";
        }
        else if (diff > 0) {
...
// CUT begin
ifstream data("../SRM 606_DIV2_1000.sample");
