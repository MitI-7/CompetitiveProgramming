...
struct WaterTank {
    ...
    bool ok(double middle) {
        double now = 0;
        FOR(i, 0, LEN(t)) {
            now += (double)t[i] * (x[i] - middle);
            if (now > C) {
                return false;
            }
            if (now < 0) {
    ...
    double minOutputRate(vector<int> _t, vector<int> _x, int _C) {
        t = _t, x = _x, C = _C;
...
// CUT begin
ifstream data("../SRM 676_DIV1_250.sample");
