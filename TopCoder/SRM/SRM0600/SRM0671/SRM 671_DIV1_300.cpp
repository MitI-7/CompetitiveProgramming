...
struct BearCries {
    ...
    int count(string _message) {
        ...
        const int N = LEN(message);
        // dp[i][j][k] = iまでみたとき，;のみの数がjで，;で閉じられる;の数がkの場合の数
        auto dp = make_v<LL>(N + 10, N + 10, N + 10);
            ...
            FOR(j, 0, N + 1) {
                FOR(k, 0, N + 1) {
                    if (message[i] == ';') {
                        ...
                        if (k > 0) {
                            (dp[i + 1][j][k - 1] += dp[i][j][k] * k) %= MOD; // 閉じる
                        }
                    }
                    else {
                        if (j > 0) {
                            (dp[i + 1][j - 1][k + 1] += dp[i][j][k] * j) %= MOD; // _を左の;に振り分ける
                        }
                        (dp[i + 1][j][k] += dp[i][j][k] * k) %= MOD; // _を左の;に振り分ける
                    }
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 671_DIV1_300.sample");
