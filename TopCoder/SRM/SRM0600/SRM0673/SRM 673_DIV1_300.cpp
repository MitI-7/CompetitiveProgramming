...
struct BearCavalry {
    vector<int> warriors;
    vector<int> horses;
    int countAssignments(vector<int> _warriors, vector<int> _horses) {
        warriors = _warriors, horses = _horses;
        const int N = LEN(warriors);
        const int bb = warriors[0];
        ...
        LL ans = 0;
        FOR(i, 0, N) {
            LL limit = bb * horses[i];

            LL c = 1;
            FOR(j, 0, LEN(warriors)) {
                int num = 0;
                FOR(k, 0, N) {
                    if (k == i) {
                        continue;
...
// CUT begin
ifstream data("../SRM 673_DIV1_300.sample");
