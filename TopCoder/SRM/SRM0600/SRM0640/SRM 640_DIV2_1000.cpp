...
struct TwoNumberGroupsEasy {
    ...
    int calc(LL mod) {
        ...
        FOR(i, 0, LEN(A)) {
            count1[A[i] % mod] += numA[i];
            ans += numA[i];
        }
        FOR(i, 0, LEN(B)) {
            count2[B[i] % mod] += numB[i];
            ans += numB[i];
        }

        FOE(p, count1) {
            if (count2[p.first] > 0) {
                ans -= min(p.second, count2[p.first]) * 2;
            }
        }
    ...
    int solve(vector<int> _A, vector<int> _numA, vector<int> _B, vector<int> _numB) {
        A = _A, numA = _numA, B = _B, numB = _numB;

        int ans = calc(10000000000);
        FOR(i, 0, LEN(A)) {
            FOR(j, 0, LEN(B)) {

                int diff = abs(A[i] - B[j]);
                if (diff > 1) {
                    FOE(d, get_divisor(diff)) {
                        if (d > 1) {
                            chmin(ans, calc(d));
                        }
                    }
...
// CUT begin
ifstream data("../SRM 640_DIV2_1000.sample");
