...
struct ProblemSets {
    ...
    bool ok(long long middle) {
        LL e = E, m = M, h = H;
        if (middle - E > 0) {
            auto use = min(EM, middle - E);
            e += use;
            m += (EM - use);
        }
        else {
    	...
    	return e >= middle and m >= middle and h >= middle;
    }

    // 開区間(low, high)で条件を満たす一番右
    // なければlowを返す
    long long binary_search_right(long long low, long long high) {
        // (low, high)
        while (high - low > 1) {
    ...
    long long maxSets(long long _E, long long _EM, long long _M, long long _MH, long long _H) {
        E = _E, EM = _EM, M = _M, MH = _MH, H = _H;

        return binary_search_right(0, LONG_LONG_MAX / 2);
    }
};

// CUT begin
ifstream data("../SRM 657_DIV1_250.sample");
