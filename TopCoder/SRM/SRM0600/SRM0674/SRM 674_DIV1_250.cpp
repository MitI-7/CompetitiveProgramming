...
struct VampireTree {
    ...
    int maxDistance(vector<int> _num) {
        num = _num;
        ...
        deque<int> que;
        FOR(i, 0, LEN(num)) {
            que.emplace_back(num[i]);
        }

        int ans = 1;
        int slot = que.back(); que.pop_back();
        while (not que.empty() and slot > 0) {

            if (slot == 1) {
                if (que.back() != 1) {
                    ans++;
                }
                slot += que.back() - 2; que.pop_back();
            }
            else {
                slot += que.front() - 2; que.pop_front();
            }

        }
        ans++;

        if (slot != 0 or not que.empty()) {
            return -1;
        }
...
// CUT begin
ifstream data("../SRM 674_DIV1_250.sample");
