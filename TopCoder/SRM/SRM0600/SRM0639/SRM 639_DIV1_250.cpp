...
struct AliceGame {
    ...
    long long binary_search_right1(long long low, long long high) {
        // (low, high)
        while (high - low > 1) {
            long long middle = (low + high) / 2;
            if (sum_of_arithmetic_progression(1, 2, middle) <= total) {
                low = middle;
            }
            else {
                high = middle;
    ...
    long long findMinimumValue(long long _x, long long _y) {
        ...
        LL turn = binary_search_right1(0, INF);
        if (sum_of_arithmetic_progression(1, 2, turn) != total) {
            return -1;
        }
        ...
        for (LL t = 0; t <= turn; ++t) {
            ...
            LL tmp1 = sum_of_arithmetic_progression(turn, -1, t);
            LL tmp2 = sum_of_arithmetic_progression(1, 1, t);
            if (tmp2 <= fixed_x and fixed_x <= tmp1) {
                return t;
            }
        }
...
// CUT begin
ifstream data("../SRM 639_DIV1_250.sample");
