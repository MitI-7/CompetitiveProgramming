...
bool is_valid_parenthesis(const std::string &parenthesis) {
    std::stack<char> s;

    for (int i = 0; i < parenthesis.size(); ++i) {
        if (not s.empty() and s.top() == '(' and parenthesis[i] == ')') {
            s.pop();
        } else {
            s.emplace(parenthesis[i]);
        }
    }
...
// CUT begin
ifstream data("../SRM 670_DIV1_250.sample");
