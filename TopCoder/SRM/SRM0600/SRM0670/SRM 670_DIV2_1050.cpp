...
std::vector<int> get_route(const int from, const int to, const Tree &tree) {
    if (from == to) {
        return {from};
    }

    // get from -> to path
    std::vector<int> prev(tree.num_of_node, -1);
    std::vector<long long> dist(tree.num_of_node);
    dist[from] = 0;
    std::stack<int> nodes;
    nodes.push(from);
...
struct Treestrat {
    ...
    int solve(const int target_node, const Tree &tree) {
        int ans = 0;
        FOR(position, 0, tree.num_of_node) {

            auto route = get_route(target_node, position, tree);

            FOR(i, 0, LEN(route)) {
                int p = route[i];
                int d = dist[target_node][p];
    ...
    int roundcnt(vector<int> tree, vector<int> _A, vector<int> _B) {
        A = _A, B = _B;

        auto N = LEN(tree) + 1;
        Tree t(N);
        FOR(i, 0, N - 1) {
            t.add_undirected_edge(tree[i], i + 1);
        }
...
// CUT begin
ifstream data("../SRM 670_DIV2_1050.sample");
