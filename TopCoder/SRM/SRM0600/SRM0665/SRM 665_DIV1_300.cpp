...
struct LuckySum {
    ...
    long long construct(string note) {
        ...
        // dp[keta][inc][mode] = keta目までみて，下位からのインクリメントがあって，0をつかった
        auto dp = make_v<LL>(N + 1, 2, 3);
        fill_v(dp, LINF);
        dp[0][0][0] = 0;
        ...
        FOR(k, 0, N) {
            FOR(inc, 0, 2) {
                ...
                if (k != 0) {
                    ...
                    // 4 + 0
                    if (c == '?' or (c - '0' == 4 + inc)) {
                        chmin(dp[k + 1][0][1], dp[k][inc][0] + ((4 + inc) * base));
                        chmin(dp[k + 1][0][1], dp[k][inc][1] + ((4 + inc) * base));
                    }
                    // 7 + 0
                    if (c == '?' or (c - '0' == 7 + inc)) {
                        chmin(dp[k + 1][0][1], dp[k][inc][0] + ((7 + inc) * base));
                        chmin(dp[k + 1][0][1], dp[k][inc][1] + ((7 + inc) * base));
                    }

                    // 両方0を使う
                    if (k == (N - 1) and (c == '?' or (c - '0' == 1))) {
                        chmin(dp[k + 1][0][2], dp[k][1][0] + (1 * base));
                        chmin(dp[k + 1][0][2], dp[k][1][1] + (1 * base));
                        chmin(dp[k + 1][0][2], dp[k][1][2] + (1 * base));
                    }
                }

                // 4 + 4
                if (c == '?' or (c - '0' == 8 + inc)) {
                    chmin(dp[k + 1][0][0], dp[k][inc][0] + ((8 + inc) * base));
                }
                // 4 + 7
                if (c == '?' or (c - '0' == 1 + inc)) {
                    chmin(dp[k + 1][1][0], dp[k][inc][0] + ((1 + inc) * base));
                }
                // 7 + 7
                if (c == '?' or (c - '0' == 4 + inc)) {
                    chmin(dp[k + 1][1][0], dp[k][inc][0] + ((4 + inc) * base));
                }
            }
            base *= 10;
        }

        LL ans = min(dp[N][0][0], min(dp[N][0][1], dp[N][0][2]));
        if (ans >= LINF) {
            return -1;
        }
        return ans;
...
// CUT begin
ifstream data("../SRM 665_DIV1_300.sample");
