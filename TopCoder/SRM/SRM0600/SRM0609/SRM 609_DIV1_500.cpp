...
struct PackingBallsDiv1 {
    ...
    int minPacks(int _K, int _A, int _B, int _C, int _D) {
        K = _K, A = _A, B = _B, C = _C, D = _D;
        ...
        LL remove = 0;
        FOR(num_variety, 1, K) {
            remove += v[num_variety];
            chmin(ans, num_variety + all_same - remove);
        }
...
// CUT begin
ifstream data("../SRM 609_DIV1_500.sample");
...
bool do_test(int K, int A, int B, int C, int D, int __expected) {
    time_t startClock = clock();
    PackingBallsDiv1 *instance = new PackingBallsDiv1();
    int __result = instance->minPacks(K, A, B, C, D);
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(K, A, B, C, D, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1591153370;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "PackingBallsDiv1 (500 Points)" << endl << endl;
    }
