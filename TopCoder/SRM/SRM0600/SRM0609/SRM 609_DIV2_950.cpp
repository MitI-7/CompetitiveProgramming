...
struct VocaloidsAndSongs {
    ...
    int count(int _S, int _gumi, int _ia, int _mayu) {
        ...
        auto dp = make_v<mint<MOD>>(S + 10, gumi + 10, ia + 10, mayu + 10);
        dp[0][0][0][0] = 1;
        ...
        FOR(i, 0, S) {
            FOR(a, 0, gumi + 1) {
                FOR(b, 0, ia + 1) {
                    FOR(c, 0, mayu + 1) {
                        // 3
                        dp[i + 1][a + 1][b + 1][c + 1] += dp[i][a][b][c];

                        // 2人
                        dp[i + 1][a + 1][b + 1][c] += dp[i][a][b][c];
                        dp[i + 1][a + 1][b][c + 1] += dp[i][a][b][c];
                        dp[i + 1][a][b + 1][c + 1] += dp[i][a][b][c];
        ...
        return dp[S][gumi][ia][mayu].x;
    }
};

// CUT begin
ifstream data("../SRM 609_DIV2_950.sample");
