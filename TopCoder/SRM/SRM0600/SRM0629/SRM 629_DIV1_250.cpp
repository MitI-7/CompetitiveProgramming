...
struct RectangleCovering {
    ...
    vector<int> boardH;
    vector<int> boardW;
    int minimumNumber(int _holeH, int _holeW, vector<int> _boardH, vector<int> _boardW) {
        holeH = _holeH, holeW = _holeW, boardH = _boardH, boardW = _boardW;
        auto N = LEN(boardH);

        int ans = INF;

        auto v1 = make_v<int>(0);
        auto v2 = make_v<int>(0);
        FOR(i, 0, N) {
            const LL h = boardH[i];
            const LL w = boardW[i];

            if (min(h, w) > holeH) {
                v1.emplace_back(max(h, w));
            }
            else if (max(h, w) > holeH) {
                v1.emplace_back(min(h, w));
            }

            if (min(h, w) > holeW) {
                v2.emplace_back(max(h, w));
            }
            else if (max(h, w) > holeW) {
                v2.emplace_back(min(h, w));
            }
        }
...
// CUT begin
ifstream data("../SRM 629_DIV1_250.sample");
