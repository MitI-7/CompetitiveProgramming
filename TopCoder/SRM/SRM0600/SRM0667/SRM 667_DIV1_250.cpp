        ...
        FOR(b, 0, LEN(dp)) {
            FOR(i, 0, N) {
                if (dp[b] != INF) {
                    int num = 0;
                    FOR(k, 0, 22) {
                        if (not IS_BIT_ON(b, k) and IS_BIT_ON(v[i], k)) {
                            num++;
                        }
                    }
                    chmin(dp[b | v[i]], dp[b] + num * num);
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 667_DIV1_250.sample");
