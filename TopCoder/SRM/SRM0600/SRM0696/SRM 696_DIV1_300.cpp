...
struct Gperm {
    ...
    int countfee(vector<int> _x, vector<int> _y) {
        x = _x, y = _y;
        unsigned int N = LEN(x);

        auto node_edge = make_v<unsigned int>(50);
        FOR(i, 0, N) {
            node_edge[x[i]] = bit_set(node_edge[x[i]], i, 1);
            if (y[i] != x[i]) {
                node_edge[y[i]] = bit_set(node_edge[y[i]], i, 1);
            }
        }

        auto dp = make_v<int>((1u << N) + 1);
        fill_v(dp, INF);
        dp[(1u << N) - 1] = 0;

        for(unsigned int b = (1u << N); b > 0; --b) {
            if (dp[b] == INF) {
                continue;
            }
            FOR(u, 0, 50) {
                unsigned int new_bit = (b & (~node_edge[u]));
                chmin(dp[new_bit], dp[b] + BIT_COUNT(b));
            }
        }
...
// CUT begin
ifstream data("../SRM 696_DIV1_300.sample");
