...
struct OthersXor {
    vector<int> x;
    long long minSum(vector<int> _x) {
        x = _x;

        const int N = LEN(x);
        sort(RALL(x));
        while (not x.empty() and x.back() == -1) {
            x.pop_back();
        }
        ...
        auto ans = make_v<LL>(N);
        for (unsigned int k = 0; k <= 31; ++k) {

            auto side_xor = make_v<unsigned int>(0);
            FOR(i, 0, LEN(x) - 1) {
                side_xor.emplace_back(is_bit_on(x[i] ^ x[i + 1], k));
            }

            bool can_set = false;
            int best_num_one = INF;
            for(unsigned int b = 0; b <= 1; ++b) {
                unsigned int total = b;
                auto temp = make_v<unsigned int>(LEN(x));
                temp[0] = b;

                FOR(j, 0, LEN(side_xor)) {
                    if (temp[j] ^ side_xor[j]) {
                        temp[j + 1] = 1;
                        total ^= 1u;
                    }
                }
                ...
                // check
                bool ok = true;
                unsigned int rest = total ^ b ^ ((x[0] >> k) & 1u);
                if (LEN(x) == N and rest != 0) {
                    ok = false;
                }
                FOR(j, 1, LEN(x)) {
                    if ((total ^ temp[j] ^ ((x[j] >> k) & 1u)) != rest) {
                        ok = false;
                    }
                }
                if (ok) {
                    ...
                    if (LEN(temp) != N and rest) {
                        num_one++;
                    }

                    if (num_one < best_num_one) {
                        FOR(j, 0, LEN(temp)) {
                            ans[j] = bit_set(ans[j], k, temp[j]);
                        }
                        if (LEN(temp) != N) {
                            ans[LEN(temp)] = bit_set(ans[LEN(temp)], k, rest > 0);
                        }
                        best_num_one = num_one;
                    }
                }
            }
...
// CUT begin
ifstream data("../SRM 699_DIV1_250.sample");
