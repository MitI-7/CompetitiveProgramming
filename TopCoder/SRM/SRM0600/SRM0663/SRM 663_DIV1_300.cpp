...
struct ABBADiv1 {
    ...
    bool dfs(string s, map<string, bool> &memo) {
        if (LEN(s) < LEN(initial)) {
            return false;
        }
    ...
    string canObtain(string _initial, string _target) {
        initial = _initial, target = _target;
        int N = LEN(target);
...
// CUT begin
ifstream data("../SRM 663_DIV1_300.sample");
