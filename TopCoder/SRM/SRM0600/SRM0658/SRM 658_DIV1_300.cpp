...
struct OddEvenTree {
            ...
            for (int s = 0; s < num_node; ++s) {
                for (int e = 0; e < num_node; ++e) {
                    ...
                    if (matrix.at(s).at(m) != INT_MAX and matrix.at(m).at(e) != INT_MAX) {
                    	if (matrix.at(s).at(e) > matrix.at(s).at(m) + matrix.at(m).at(e)) {
                            matrix.at(s).at(e) = matrix.at(s).at(m) + matrix.at(m).at(e);
                        }
                    }
                }
            }
    ...
    vector<int> getTree(vector<string> _x) {
        x = _x;
        int N = LEN(x);
        auto ans = make_v<int>(0);
        std::vector<std::vector<int>> matrix(N, vector<int>(N, INT_MAX));
        int odd = -1;
        FOR(i, 1, N) {
            if (x[0][i] == 'O') {
                ans.emplace_back(0);
        ...
        auto check = warshall_floyd(matrix);
        FOR(i, 0, N) {
            FOR(j, 0, N) {
                if (x[i][j] == 'E' and check[i][j] % 2 == 1) {
                    return {-1};
                }
                if (x[i][j] == 'O' and check[i][j] % 2 == 0) {
                    return {-1};
                }
            }
        }
...
// CUT begin
ifstream data("../SRM 658_DIV1_300.sample");
