...
struct ParenthesesDiv1Easy {
    ...
    vector<int> correct(string _s) {
        ...
        auto stack = make_v<pair<char, int>>(0);
        FOR(i, 0, N) {
            if (not stack.empty() and stack.back().first == '(' and s[i] == ')') {
                stack.pop_back();
            }
            else {
                stack.emplace_back(make_pair(s[i], i));
            }
        }
        ...
        FOR(i, 0, LEN(stack)) {
            num_left += stack[i].first == '(';
            num_right += stack[i].first == ')';
            if (stack[i].first == ')') {
                last_right_idx = i;
            }
        }
        int half = LEN(stack) / 2;

        auto ans = make_v<int>(0);
        if (num_left == num_right) {
            ans.emplace_back(stack[0].second);
            ans.emplace_back(stack[last_right_idx].second);
            ans.emplace_back(stack[last_right_idx + 1].second);
            ans.emplace_back(stack.back().second);
        }
        else if (num_right > num_left) {
            ans.emplace_back(stack[0].second);
            ans.emplace_back(stack[half].second - 1);
            if (num_left != 0) {
                ans.emplace_back(stack[last_right_idx + 1].second);
                ans.emplace_back(stack.back().second);
            }
            ...
            ans.emplace_back(stack[LEN(stack) - half].second);
            ans.emplace_back(stack.back().second);
        }
...
// CUT begin
ifstream data("../SRM 688_DIV1_250.sample");
