...
struct CliqueParty {
    ...
    LL func(int b, int e, LL diff) {
        auto dp = make_v<int>(55, 55);
        dp[b + 1][b] = 1;
    ...
    int maxsize(vector<int> _a, int _k) {
        ...
        FOR(i, 0, N) {
            FOR(j, i + 1, N) {
                LL max_diff = a[j] - a[i];
                LL min_diff = ceil(max_diff, k);

                chmax(ans, func(i, j + 1, min_diff));
            }
        }
...
// CUT begin
ifstream data("../SRM 684_DIV1_250.sample");
