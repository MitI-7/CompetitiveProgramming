...
struct CatsOnTheLineDiv1 {
    ...
    int getNumber(vector<int> _position, vector<int> _count, int _time) {
        position = _position, count = _count, time = _time;
        auto N = LEN(position);
        auto v = make_v<pair<int, int>>(N);
        FOR(i, 0, N) {
        ...
        int ans = 0;
        int last_multi_cat = -INF;
        int last_cat = -INF;
        FOR(i, 0, N) {
            int now_position = v[i].first;
            int now_num_cat = v[i].second;

            // multi catに到達できる
            if (time >= abs(last_multi_cat - now_position)) {
                continue;
            }

            // なるべく左につめておく
            if (last_cat < now_position) {
                int space = now_position - last_cat - 1;
                now_num_cat -= min(time, space);

                if (now_num_cat <= 0) {
                    last_cat = now_position + now_num_cat - 1;
                    continue;
                }
            }
            ...
            int rest_time = time;
            if (last_cat >= now_position) {
                int need_move = last_cat - now_position + 1;
                if (need_move > time) {
                    ans++;
                    last_multi_cat = now_position + time;
                    continue;
                }
                rest_time -= need_move;
                now_position = last_cat + 1;
            }

            // 右にも設置
            // 絶対重なっちゃう
            if (now_num_cat - 1 > rest_time) {
                ans++;
                last_multi_cat = last_cat = now_position + rest_time;
            }
            else {
                last_cat = now_position + now_num_cat - 1;
            }
...
// CUT begin
ifstream data("../SRM 631_DIV1_500.sample");
...
bool do_test(vector<int> position, vector<int> count, int time, int __expected) {
