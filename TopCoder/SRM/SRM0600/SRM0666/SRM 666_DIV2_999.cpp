...
struct CollectingTokens {
    ...
    vector<vector<vector<int>>> dp; // dp[u][i][j] = uを頂点とする部分木から開始するとき，i移動してiにj(0: 戻らない，1:戻った)の最大トークン数
    ...
    void dfs(int u, int parent) {
        ...
        FOE(v, tree[u]) {
            ...
            for (int i = L; i >= 0; --i) {
                for (int j = L; j >= 0; --j) {
                    chmax(dp[u][i + j + 1][0], dp[u][i][1] + dp[v][j][0]);  // v方向に行ったきり戻らない
                    chmax(dp[u][i + j + 2][0], dp[u][i][0] + dp[v][j][1]);  // vからは戻るが，他のノードで戻らない
                    chmax(dp[u][i + j + 2][1], dp[u][i][1] + dp[v][j][1]);  // 戻る
                }
            }
        }
    }

    int maxTokens(vector<int> _A, vector<int> _B, vector<int> _tokens, int _L) {
        A = _A, B = _B, tokens = _tokens, L = _L;
        int N = LEN(A);

        tree = make_v<int>(101, 0);
        FOR(i, 0, LEN(A)) {
            tree[A[i] - 1].emplace_back(B[i] - 1);
            tree[B[i] - 1].emplace_back(A[i] - 1);
        }
        dp = make_v<int>(101, 300, 2);
...
// CUT begin
ifstream data("../SRM 666_DIV2_999.sample");
