...
struct WalkOverATree {
    ...
    int maxNodesVisited(vector<int> _parent, int _L) {
        parent = _parent, L = _L;

        int N = LEN(parent) + 1;
        this->dist = make_v<int>(N);
        this->children = make_v<int>(N, 0);

        FOR(i, 0, LEN(parent)) {
            children[parent[i]].emplace_back(i + 1);
        }
        ...
        FOR(last, 0, N) {
            ...
            int num_node = dist[last] + 1;
            int rest = N - num_node;
            chmax(ans, num_node + min((L - dist[last]) / 2, rest));
        }
...
// CUT begin
ifstream data("../SRM 666_DIV1_222.sample");
