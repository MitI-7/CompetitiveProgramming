...
struct TreeAndPathLength2 {
    ...
    void dfs(int num_node, int num_leaf, int num_path, vector<vector<vector<bool>>> &ok) {
        if (ok[num_node][num_leaf][num_path]) {
            return;
        }

        ok[num_node][num_leaf][num_path] = true;

        FOR(i, 1, n) {
            int new_num_path = num_path + i + (i * (i - 1)) / 2;
            if (num_node + i <= n and new_num_path <= s) {
                dfs(num_node + i, num_leaf - 1 + i, new_num_path, ok);
            }
        }
    }

    string possible(int _n, int _s) {
        n = _n, s = _s;

        auto ok = make_v<bool>(n + 10, n + 10, s + 10);
        FOR(u, 2, n + 1) {
            if (((u - 1) * (u - 2)) / 2 <= s) {
                dfs(u, u - 1, ((u - 1) * (u - 2) / 2), ok);
            }
        }
...
// CUT begin
ifstream data("../SRM 675_DIV2_900.sample");
