...
struct TreeAndPathLength3 {
    ...
    tuple<int, int, int> func(int s) {
        ...
        FOR(x, 1, 300) {
            ...
            int need_node = x * 2 + 1 + y + extra;

            if (need_node <= 300) {
                return make_tuple(x, y, extra);
            }
        }
    ...
    vector<int> construct(int s) {
        ...
        int node = 1;
        int x, y, extra;
        tie(x, y, extra) = func(s);

        FOR(i, 0, y) {
            ans.emplace_back(0); ans.emplace_back(node++);
        }

        FOR(i, 0, x) {
            ans.emplace_back(0); ans.emplace_back(node);
            ans.emplace_back(node++); ans.emplace_back(node++);
        }
...
// CUT begin
ifstream data("../SRM 675_DIV1_300.sample");
