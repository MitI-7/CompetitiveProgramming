...
void dfs(long long u, long long c, std::vector<long long> &color, vector<vector<LL>> &graph) {
    color[u] = c;
...
LL solve(LL N, LL M, LL C_lib, LL C_road, vector<vector<LL>> &graph) {
    std::vector<long long> color(N, -1);
    long long num_color = 0;
    for (long long u = 0; u < N; ++u) {
        if (color[u] == -1) {
            dfs(u, num_color, color, graph);
            num_color++;
        }
    }
    ...
    FOR(c, 0, num_color) {
        ...
        FOR(i, 0, num_node) {
            chmin(tmp_ans, i * C_road + (num_node - i) * C_lib);
        }
        ans += tmp_ans;
    }
...
int main() {
    ...
    FOR(_, 0, Q) {
        ...
        print(solve(N, M, C_lib, C_road, graph));
    }
