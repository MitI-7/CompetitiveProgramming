...
LL left(int begin, int end, LL x, SegmentTree<LL> &seg) {
    if (seg.query(begin, end) <= x) {
        return 0;
    }
...
LL right(int begin, int end, LL x, SegmentTree<LL> &seg) {
    if (seg.query(begin, end) <= x) {
        return 0;
    }
...
LL solve(int N, const vector<LL> &A) {
    ...
    LL ans = 0;
    FOR(i, 1, N - 1) {
        auto l = left(0, i, A[i], seg);
        auto r = right(i + 1, N, A[i], seg);
        chmax(ans, l * r);
    }
