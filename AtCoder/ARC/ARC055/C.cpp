...
LL solve(string S) {
    ...
    FOR(i, 3, N - 1) {
        const int len_l = i;
        const int len_r = N - len_l;
        const int len_b = len_l - len_r;
        if (len_b <= 0) {
            continue;
        }

        if (z[i] + rev_z[i - 1] < len_r) {
            continue;
        }

        const int max_c = min(rev_z[i - 1], len_r - 1);
        int min_a = min(z[i], len_r - max_c);
        int max_a = min(z[i], len_r - 1);
