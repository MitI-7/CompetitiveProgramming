...
// 頂点を1と-1で塗っていく
template<typename T>
bool dfs(const int u, const int color, std::vector<int> &colors, vector<bool> &used, const Graph<T> &graph) {
    colors[u] = color;
    used[u] = true;
    for (auto edge: graph.graph[u]) {
        if (colors[edge.to] == color) {
            ...
            if (dfs(edge.to, -color, colors, used, graph) == false) {
                return false;
            }
        }
    }
...
int main() {
    ...
    // dp[i][s] = iまでみて，合計がsを作れるかどうか
    auto dp = make_v<int>(LEN(v) + 1, 1000);
    fill_v(dp, false);
    dp[0][0] = true;
    FOR(i, 0, LEN(v)) {
        const auto [a, b] = v[i];
