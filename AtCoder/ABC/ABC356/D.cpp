...
// N 以下で b 桁目の bit が立つような場合の数
mint<MOD> count_number_with_bth_bit(const long long N, const int b) {
    const auto bit = static_cast<unsigned long long>(1) << b;
    const auto cycle = bit * 2;
    const auto full_cycles = (N + 1) / cycle;
    const auto remainder = (N + 1) % cycle;
    auto count = mint<MOD>(full_cycles) * bit;
    if (remainder > bit) {
        count += remainder - bit;
    }
    return count;
...
int main() {
    ...
    mint<MOD> ans;
    FOR(b, 0, 64) {
        if (not is_bit_on(M, b)) {
            continue;
        }
        ans += count_number_with_bth_bit(N, b);
    }
    print(ans);
