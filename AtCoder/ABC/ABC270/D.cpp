...
int takahashi(int i, int rest, const vector<int> &A) {
    if (rest == 0) {
        return 0;
    }
    if (dp[i][rest] != -1) {
    ...
    if (i % 2 == 0) {
        ...
        FOE(a, A) {
            ...
            chmax(ans, takahashi((i + 1) % 2, rest - a, A) + a);
        }
        ...
        FOE(a, A) {
            ...
            chmin(ans, takahashi((i + 1) % 2, rest - a, A));
        }
        return dp[i][rest] = ans;
    }
}
...
int main() {
    ...
    dp = make_v<int>(2, N + 10);
    fill_v(dp, -1);
    
    print(takahashi(0, N, A));
