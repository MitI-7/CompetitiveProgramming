...
int main() {
    ...
    FOR(i, 0, Q) {
        ...
        cin >> D >> L;
        if (D == 'L') {
            auto it = ys[now_y].lower_bound(now_x);
            // 左はない
            if (it == ys[now_y].begin()) {
                now_x = max(1, now_x - L);
            } else {
                it--;
                now_x = max(*it + 1, now_x - L);
            }
        } else if (D == 'R') {
            auto it = ys[now_y].lower_bound(now_x);
            // 右はない
            if (it == ys[now_y].end()) {
                ...
                now_x = min(*it - 1, now_x + L);
            }
        } else if (D == 'U') {
            auto it = xs[now_x].lower_bound(now_y);
            // 上はない
            if (it == xs[now_x].begin()) {
                now_y = max(1, now_y - L);
            } else {
                it--;
                now_y = max(*it + 1, now_y - L);
            }
        } else if (D == 'D') {
            auto it = xs[now_x].lower_bound(now_y);
            // 下はない
