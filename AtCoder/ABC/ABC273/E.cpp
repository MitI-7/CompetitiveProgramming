...
private:
    ...
    template<class... Args>
    PersistentStack push_p(Args &&... args) const {
        return PersistentStack(make_node(root, ::std::forward<Args>(args)...));
    }

    explicit PersistentStack(node_ptr &&r) : root(::std::move(r)) {}

    explicit PersistentStack(const node_ptr &r) : root(r) {}

public:
    constexpr PersistentStack() noexcept: root() {}
    ...
    PersistentStack push(const value_type &x) const { return push_p(x); }

    PersistentStack push(value_type &&x) const { return push_p(::std::move(x)); }
...
int main() {
    ...
    vector<int> ans(Q);
    PersistentStack<int> now;
    map<int, PersistentStack<int>> save;
    ...
    for (int i = 0; i < Q; ++i) {
        ...
        if (QUERY == "ADD") {
            ...
            cin >> Z;
            if (save.find(Z) == save.end()) {
                now = PersistentStack<int>();
            } else {
                now = save[Z];
            }
        }
