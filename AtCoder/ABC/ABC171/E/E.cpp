...
int main() {
    ...
    auto ans = make_v<unsigned long long>(N);
    FOR(b, 0, 32) {

        FOR(f, 0, 2) {
            auto tmp = make_v<unsigned long long>(N);
            tmp[0] = f;
            FOR(i, 1, N) {
                auto x = A[i - 1] ^ A[i];
                tmp[i] = (is_bit_on(x, b) ^ tmp[i - 1]);
            }
            ...
            bool ok = true;
            FOR(i, 0, N) {
                ok &= (is_bit_on(A[i], b) == (entire_xor ^ tmp[i]));
            }

            if (ok) {
                FOR(i, 0, N) {
                    ans[i] = bit_set(ans[i], b, tmp[i]);
                }
                break;
            }
