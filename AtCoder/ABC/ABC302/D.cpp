...
private:
    std::vector<T> a;
public:
    explicit SortedVectorUtility(const std::vector<T> &a) : a(a) {}
    ...
    int count_lte_range(const int l, const int r, T x) const {
        if (l == r) {
            return 0;
        }
        return std::upper_bound(a.begin() + l, a.begin() + r, x) - (a.begin() + l);
    }

    // 区間[l, r)に存在する x 以上の個数
    int count_gte_range(const int l, const int r, T x) const {
        if (l == r) {
            return 0;
        }
        const auto it = std::lower_bound(a.begin() + l, a.begin() + r, x);
        return (a.begin() + r) - it;
    }
...
int main() {
    ...
    SortedVectorUtility<long long> svu(B);

    long long ans = -1;
    for (const auto a: A) {
        const auto idx = svu.index_lte(a + D);
        if (idx != -1) {
            if (abs(B[idx] - a) <= D) {
                ans = max(ans, B[idx] + a);
            }
