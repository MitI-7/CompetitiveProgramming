...
mint<MOD> dfs(int u, int p, vector<vector<int>> &graph) {
    mint<MOD> ans = 1;
    FOE(v, graph[u]) {
        if (p != v) {
            ans *= dfs(v, u, graph);
...
int main() {
    ...
    set<pair<int, int>> uncover;
    for (int i = N - 1; i >= 0; --i) {
        LL X, D;
        tie(X, D) = x_d[i];

        while (not uncover.empty() and uncover.begin()->first < X + D) {
            graph[i].emplace_back(uncover.begin()->second);
            uncover.erase(uncover.begin());
        }
        uncover.emplace(make_pair(X, i));
    }

    mint<MOD> ans = 1;
    FOE(p, uncover) {
        ans *= dfs(p.second, -1, graph);
    }
    print(ans.x);
