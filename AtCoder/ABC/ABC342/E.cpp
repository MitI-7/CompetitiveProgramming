...
int main() {
    ...
    vector<vector<tuple<LL, LL, LL, LL, int>>> in(N);
    FOR(i, 0, M) {
        LL L, D, K, C, A, B;
        cin >> L >> D >> K >> C >> A >> B;
        A--;
        B--;
        in[B].emplace_back(L, D, K, C, A);
    }

    vector ans(N, -LINF);
    std::vector<bool> seen(N);

    MaxPriorityQueue<pair<LL, int>> que;
    que.emplace(LINF, N - 1);

    while (not que.empty()) {
        auto [time_limit, u] = que.top();
        que.pop();
        if (seen[u]) {
            continue;
        }
        ...
        for (auto [l, d, k, c, a] : in[u]) {
            ...
            auto ok = [&](long long i) {
                ...
                return l + i * d + c <= time_limit;
            };

            // 条件を満たす最大値を区間(left, right)から探索
            // o o o o o o o x x x x x x x の一番右の o を探す
