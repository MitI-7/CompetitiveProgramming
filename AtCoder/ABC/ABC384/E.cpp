...
int main() {
    ...
    MinPriorityQueue<tuple<LL, int, int>> que;
    FOR(i, 0, 4) {
        const int ny = P + dy8[i];
        const int nx = Q + dx8[i];
        if (inside(ny, nx, H, W)) {
            used[ny][nx] = true;
    ...
    while (not que.empty()) {
        ...
        FOR(i, 0, 4) {
            ...
            if (inside(ny, nx, H, W) and not used[ny][nx]) {
                used[ny][nx] = true;
                que.emplace(grid[ny][nx], ny, nx);
            }
        }
