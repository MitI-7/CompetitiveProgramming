...
int main() {
    ...
    LL H, W;
    cin >> H >> W;
    auto f = make_v<string>(H);
    FOR(i, 0, H) {
        cin >> f[i];
    }

    auto dp = make_v<mint<MOD>>(H, W);

    // 0:右 1:右下 2:下
    auto pr = make_v<mint<MOD>>(H, W, 3);
    dp[0][0] = 1;

    FOR(y, 0, H) {
        FOR(x, 0, W) {
            dp[y][x] += pr[y][x][0] + pr[y][x][1] + pr[y][x][2];

            if (inside(y, x + 1, H, W) and f[y][x + 1] != '#') {
                pr[y][x + 1][0] += pr[y][x][0] + dp[y][x];
            }
            if (inside(y + 1, x + 1, H, W) and f[y + 1][x + 1] != '#') {
                pr[y + 1][x + 1][1] += pr[y][x][1] + dp[y][x];
            }
            if (inside(y + 1, x, H, W) and f[y + 1][x] != '#') {
                pr[y + 1][x][2] += pr[y][x][2] + dp[y][x];
            }
        }
    }
