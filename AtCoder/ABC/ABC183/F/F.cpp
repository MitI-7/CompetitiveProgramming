...
int main() {
    ...
    // memo[集団][クラス] = 人数
    auto memo = make_v<map<int, int>>(N);
    FOR(i, 0, N) {
        memo[i][C[i]]++;
    }
    ...
    FOR(_, 0, Q) {
        ...
        if (T == 1) {
            ...
            const auto a = uf.find_root(A);
            const auto b = uf.find_root(B);
            ...
            // aの方にマージ
            if (memo[b].size() < memo[a].size()) {
                FOE(p, memo[b]) {
                    memo[a][p.first] += p.second;
                }
                swap(memo[uf.find_root(A)], memo[a]);
            }
            // bの方にマージ
            else {
                FOE(p, memo[a]) {
                    memo[b][p.first] += p.second;
                }
                swap(memo[uf.find_root(B)], memo[b]);
            }
        }
        else {
            int X, Y;
