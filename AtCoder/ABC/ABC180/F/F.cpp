...
Combination comb(1000, MOD);

mint<MOD> f(LL N, LL M, LL L) {

    auto path = make_v<mint<MOD>>(1000);
    path[1] = path[2] = 1;
    FOR(i, 3, 1000) {
        path[i] = path[i - 1] * i;
    }

    auto dp = make_v<mint<MOD>>(N + 100, M + 100);
    dp[0][0] = 1;
    ...
    FOR(i, 0, N + 1) {
        FOR(j, 0, M + 1) {
            FOR(k, 1, L + 1) {
                ...
                // path graph
                if (N - i - 1 >= k - 1 and i + k <= N and j + k - 1 <= M) {
                    dp[i + k][j + k - 1] += dp[i][j] * comb.nCr(N - i - 1, k - 1) * path[k];
                }
                ...
                if (i + k <= N and j + k <= M) {
                    dp[i + k][j + k] += dp[i][j] * comb.nCr(N - i - 1, k - 1) * path[k - 1];
                }
            }
        }
    }
