...
int main() {
    ...
    fill_v(dp, LINF);
    dp[bit_set(0, 0, 1)][0] = 0;

    FOR(b, 0, (1 << N)) {
        FOR(i, 0, N) {
            FOR(j, 0, N) {
                if (not is_bit_on(b, j)) {
                    auto new_b = bit_set(b, j, 1);
                    auto cost = abs(X[j] - X[i]) + abs(Y[j] - Y[i]) + max(0LL, Z[j] - Z[i]);
                    chmin(dp[new_b][j], dp[b][i] + cost);
                }
            }
        }
    }
    ...
    LL ans = LINF;
    FOR(i, 1, N) {
        chmin(ans, dp[(1 << N) - 1][i] + abs(X[0] - X[i]) + abs(Y[0] - Y[i]) + max(0LL, Z[0] - Z[i]));
    }
    print(ans);
