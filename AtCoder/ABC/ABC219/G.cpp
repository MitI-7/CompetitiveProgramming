...
int main() {
    ...
    FOR(u, 0, N) {
        if (deg[u] > static_cast<int>(sqrt(N))) {
            is_big[u] = true;
        }
    }

    Graph to_small(N), to_big(N);
    for (auto [u, v]: edges) {
        // u -> v
        if (is_big[v]) {
            to_big.add_directed_edge(u, v);
        } else {
            to_small.add_directed_edge(u, v);
        }

        // v -> u
        if (is_big[u]) {
            to_big.add_directed_edge(v, u);
        } else {
            to_small.add_directed_edge(v, u);
        }
    }

    vector<pair<int, int>> v(N), update(N);
    FOR(u, 0, N) {
        v[u] = {u, -1};
        update[u] = {-INF, -INF};
    }
    ...
    FOR(i, 0, Q) {
        ...
        // 自分を最新にする
        FOE(e, to_big[X]) {
            if (update[e.to].second >= v[X].second) {
                v[X] = update[e.to];
            }
        }

        if (is_big[X]) {
            update[X] = {v[X].first, i};
        } else {
            const int value = v[X].first;
            v[X] = {value, i};
            FOE(e, to_small[X]) {
                v[e.to] = {value, i};
            }
    ...
    FOR(u, 0, N) {
        FOE(e, to_big[u]) {
            if (update[e.to].second >= v[u].second) {
                v[u] = update[e.to];
            }
        }
    }
