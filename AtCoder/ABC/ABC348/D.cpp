...
int main() {
    ...
    MaxPriorityQueue<tuple<int, int, int>> que;
    que.emplace(energy[sy][sx], sy, sx);

    while (not que.empty()) {
        const auto [e, y, x] = que.top();
        que.pop();
        ...
        FOR(i, 0, 4) {
            ...
            if (inside(ny, nx, H, W) and grid[ny][nx] != '#') {
                ...
                int ne = e - 1;
                chmax(ne,  energy[ny][nx]);
                if (dp[ny][nx] < ne) {
                    que.emplace(ne, ny, nx);
                }
            }
        }
    }
