...
vector<int> solve(int N, vector<pair<int, int>> &a, vector<pair<int, int>> &b) {
    UnionFind uf(N), uf1(N), uf2(N);
    for (auto [u, v] : a) {
        uf1.unite(u, v);
    }
    ...
    FOR(i, 0, N) {
        m[{uf1.leader(i), uf2.leader(i)}]++;
    }

    vector<int> ans(N);
    FOR(i, 0, N) {
        ans[i] = m[{uf1.leader(i), uf2.leader(i)}];
    }
    return ans;
}
