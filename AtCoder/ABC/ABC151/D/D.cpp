...
LL solve(int y1, int x1, int y2, int x2, const vector<vector<char>> &grid) {
    int H = LEN(grid);
    int W = LEN(grid[0]);

    queue<pair<int, int>> que;
    que.emplace(make_pair(y1, x1));

    auto cost = make_v<int>(H, W);
    fill_v(cost, INF);
    cost[y1][x1] = 0;
        ...
        FOR(x1, 0, W) {
            FOR(y2, 0, H) {
                FOR(x2, 0, W) {
                    ...
                    chmax(ans, solve(y1, x1, y2, x2, grid));
                }
            }
        }
    }
