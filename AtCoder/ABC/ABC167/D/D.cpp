...
public:
    // n:function(x)のxの最大値
    Doubling(int n, long long max_k) : LOG(64 - __builtin_clzll(max_k)) {
        this->table.assign(LOG, std::vector<T>(n, -1));
    }

    // xの遷移先
    // function(x) = y
    void function(int x, T y) {
        this->table[0][x] = y;
    }
    ...
    void build() {
        for (int k = 0; k + 1 < this->LOG; ++k) {
            for (int i = 0; i < this->table[k].size(); ++i) {
                if (this->table[k][i] == -1) {
                    this->table[k + 1][i] = -1;
                } else {
...
int main() {
    ...
    Doubling<LL> doubling(N, K);

    FOR(i, 0, N) {
        int A;
        cin >> A;
        A--;
        doubling.function(i, A);
    }

    doubling.build();
    print(doubling.query(0, K) + 1);
