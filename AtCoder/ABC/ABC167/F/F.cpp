...
pair<int, int> func(const string &p) {
    stack<char> s;
    FOR(i, 0, LEN(p)) {
        if (not s.empty() and s.top() == '(' and p[i] == ')') {
            s.pop();
        }
        else {
            s.emplace(p[i]);
...
bool solve(vector<string> &S) {
    int left = 0;
    int right = 0;
    auto pos = make_v<pair<int, int>>(0);
    auto neg = make_v<pair<int, int>>(0);
    FOR(i, 0, LEN(S)) {
        auto p = func(S[i]);
        int l = p.first;
        int r = p.second;
        ...
        if (l > 0 and r == 0) {
            ...
            if (l >= r) {
                pos.emplace_back(make_pair(r, l));
            }
            else {
                neg.emplace_back(make_pair(r, l));
            }
        }
    }
