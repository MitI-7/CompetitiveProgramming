...
template<typename T>
bool dfs(const int u, const Edge<T> &prev, std::vector<int> &state, std::vector<Edge<T>> &edges, const Graph<T> &graph) {
    state[u] = 1;
...
void dfs2(int u, int p, int c, vector<int> &color, Graph<int> &g) {
    color[u] = c;
    FOE(e, g.graph[u]) {
        if (e.to == p) {
            continue;
        }
        dfs2(e.to, u, c, color, g);
    }
}
...
int main() {
    ...
    FOE(e, cycle) {
        in_cycle[e.from] = in_cycle[e.to] = true;
    }
    ...
    FOR(u, 0, N) {
        if (in_cycle[u]) {
            FOE(e, graph[u]) {
                color[u] = c;
                if (not in_cycle[e.to]) {
                    dfs2(e.to, u, c, color, graph);
                }
            }
            c++;
        }
