...
// middle以上の個数
LL count(long long middle, vector<LL> &A) {
    LL num = 0;
    FOR(i, 0, LEN(A)) {
        // x以上の個数
        num += A.end() - lower_bound(A.begin(), A.end(), middle - A[i]);
    }
    return num;
}
...
int main() {
    ...
    CumulativeSum<LL> cs; cs.build(A);
    LL ans = 0;
    // 足してhighになるやつはとっていい
    FOR(i, 0, N) {
        LL num = A.end() - lower_bound(A.begin(), A.end(), high - A[i]);
        ans += A[i] * num;
        ans += cs.sum(N - num, N);
    }

    const LL rest = M - count(high, A);
