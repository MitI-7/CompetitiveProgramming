...
int main(int argc, char *argv[]) {
    ...
    auto left = make_v<LL>(0);
    auto right = make_v<LL>(0);
    left.emplace_back(0);
    ...
    last = INF;
    for (int i = N - 1; i >= 0; --i) {
        if (S[i] == 'o' and abs(last - i) > C) {
            last = i;
            right.emplace_back(right.back() + 1);
        }
        else {
            right.emplace_back(right.back());
        }
    }
    reverse(ALL(right));
