...
Combination<MOD> comb(1000100);

// 縦h,横wのbounding boxとなるようなk個のマスの選び方
mint<MOD> calc(const int h, const int w, const int K) {
    ...
    FOR(b, 0, 1 << 4) {
        ...
        if (BIT_COUNT32(b) % 2 == 1) {
            ans -= comb.nCr(nh * nw, K);
        }
        else {
            ans += comb.nCr(nh * nw, K);
        }
...
int main() {
    ...
    FOR(h, 1, H + 1) {
        FOR(w, 1, W + 1) {
            mint<MOD> a = (H - h + 1) * (W - w + 1);    // bounding box の個数
            mint<MOD> b = h * w;                        // bounding box 内のマスの個数
            ans += calc(h, w, K) * a * b;
        }
    }
    ans /= comb.nCr(H * W, K);
