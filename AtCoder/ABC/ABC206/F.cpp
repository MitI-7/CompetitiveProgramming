...
int dfs(pair<int, int> range, vector<pair<int, int>> &v) {

    if (memo.contains(range)) {
        return memo[range];
    }
    set<int> s;
    for (auto [l, r]: v) {
        if (range.first <= l and r <= range.second) {
            pair<int, int> range1 = {range.first, l - 1};
            pair<int, int> range2 = {r + 1, range.second};
            int n1 = dfs(range1, v);
            int n2 = dfs(range2, v);
            s.insert(n1 ^ n2);
        }
