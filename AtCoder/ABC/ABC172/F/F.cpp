...
long long solve(vector<unsigned long long> &A) {
    ...
    const unsigned long long total = A[0] + A[1];
    const unsigned long long t = total - x;
    if (x > total or t % 2 != 0) {
        return -1;
    }
    unsigned long long a_and_b = t / 2;
    unsigned long long ans = a_and_b;
    for (int b = 50; b >= 0; --b) {
        if (is_bit_on(x, b)) {
            if (bit_set(ans, b, 1) <= A[0]) {
                ans = bit_set(ans, b, 1);
            }
        }
    }
    ...
    LL move = A[0] - ans;
    if (((A[0] - move) ^ (A[1] + move) ^ x) != 0) {
        return -1;
    }
    if (move < 0 or A[0] - move < 1) {
        return -1;
    }
    else {
        return move;
...
int check(vector<unsigned long long> &A) {
    unsigned long long entire_xor = 0;
    FOE(a, A) {
        entire_xor ^= a;
    }
    if (entire_xor == 0) {
...
int main() {
    ...
    LL N;
    cin >> N;
    auto A = make_v<unsigned long long>(N);
    FOR(i, 0, N) {
        cin >> A[i];
    }
