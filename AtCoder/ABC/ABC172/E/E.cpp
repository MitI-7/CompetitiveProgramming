...
public:
    ...
    long long nCr(int n, int r) {
        assert(0 <= n and n < this->fact.size() and 0 <= r);
        assert(n >= r);
        auto a = this->fact[n];
        auto b = (this->inv[r] * this->inv[n - r]) % this->mod;
        return (a * b) % this->mod;
    }
...
// n == mのとき，完全順列
// O(n)
long long derangement(int n, int m, int MOD) {

    Combination comb(std::max(n, m) + 100, MOD);

    long long ans = 0;
    for (int i = 0; i <= n; ++i) {
        // i個以上の箇所が指定の色と一致してしまっている
        long long now = comb.nCr(n, i);
...
int main() {
    ...
    Combination comb(M + 100, MOD);

    print(comb.nPr(M, N) * derangement(N, M, MOD) % MOD);
