...
int main() {
    ...
    que.emplace(sy, sx, 1, 0);

    while (not que.empty()) {
        const auto [y, x, d, c] = que.front();
        que.pop();
        ...
        FOR(i, 0, 4) {
            ...
            if (inside(ny, nx, H, W) and grid[ny][nx] != '#') {
                // 右 or 左
                if ((i == 0 or i == 2) and d == 0) {
                    if (dp[ny][nx][1 - d] > c + 1) {
                        dp[ny][nx][1 - d] = c + 1;
                        que.emplace(ny, nx, 1 - d, c + 1);
                    }
                }

                // 上 or 下
                if ((i == 1 or i == 3) and d == 1) {
                    if (dp[ny][nx][1 - d] > c + 1) {
                        dp[ny][nx][1 - d] = c + 1;
                        que.emplace(ny, nx, 1 - d, c + 1);
                    }
    ...
    LL ans = min(dp[gy][gx][0], dp[gy][gx][1]);
    if (ans == INF) {
        ans = -1;
    }
    print(ans);
