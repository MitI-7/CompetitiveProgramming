...
std::vector<std::vector<std::vector<bool>>> found;

template<typename T>
T digit_dp(const int digit, const bool is_tight, const bool leading_zero, const int head, const std::string &S) {
    if (digit >= static_cast<int>(S.size())) {
        // 全部0のときの処理
        if (leading_zero) {
            return 0;
        }
    ...
    if (found[digit][is_tight][leading_zero]) {
        return dp[digit][is_tight][leading_zero];
    }

    int upper = head - 1;
    if (is_tight) {
        upper = min(S[digit] - '0', head - 1);
    }

    if (leading_zero) {
        if (is_tight) {
            upper = min(S[digit] - '0', head);
        }
        else {
            upper = head;
        }
    ...
    for (int i = 0; i < upper + 1; ++i) {
        if (leading_zero and i != 0 and i != head) {
            continue;
        }

        ans += digit_dp<T>(digit + 1,
                           i == (S[digit] - '0') and is_tight,// tight が続く
                           leading_zero and i == 0,// leading zero が続く
                           head,
                           S);
    }

    found[digit][is_tight][leading_zero] = true;
    return dp[digit][is_tight][leading_zero] = ans;
}
...
LL solve(string& S) {
    ...
    LL ans = 0;
    FOR(head, 1, 10) {
        dp = make_v<long long>(N, 2, 2);
        found = make_v<bool>(N, 2, 2);
        ans += digit_dp<LL>(0, true, true, head, S);
    }
    return ans;
}
