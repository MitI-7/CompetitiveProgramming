...
LL dfs(int y, int x, int K, set<pair<int, int>> &used, vector<string> &grid) {
    const int H = LEN(grid);
    const int W = LEN(grid[0]);
    if (K == 0) {
        return 1;
    }
    ...
    FOR(i, 0, 4) {
        ...
        if (inside(ny, nx, H, W) and grid[ny][nx] == '.' and not used.contains({ny, nx})) {
            ans += dfs(ny, nx, K - 1, used, grid);
        }
    }
    used.erase({y, x});
