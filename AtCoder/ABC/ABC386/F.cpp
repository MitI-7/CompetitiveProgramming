...
int levenshtein_distance(const std::string &a, const std::string &b) {
    std::vector dp(2, std::vector<LL>(b.size() + 1, INF));
    ...
    for (int i = 1; i < static_cast<int>(a.size()) + 1; ++i) {
        dp[i % 2][0] = i;
        for (int j = max(1, i - 25); j < min(i + 25, static_cast<int>(b.size()) + 1); ++j) {
            int x;
            if (a[i - 1] == b[j - 1]) {
                x = 0;
            }
            ...
            dp[i % 2][j] = std::min(std::min(dp[(i - 1) % 2][j] + 1, dp[i % 2][ j - 1] + 1), dp[(i - 1) % 2][j - 1] + x);
        }
    }
