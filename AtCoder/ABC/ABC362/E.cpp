...
int main() {
    ...
    auto ans = make_v<mint<MOD>>(N);
    ans[0] = N;
    FOE(d, diff) {
        // dp[i][l] = A[i]を最後としたとき，長さlの等差数列の個数
        auto dp = make_v<mint<MOD>>(N, N + 1);

        vector<unordered_map<LL, mint<MOD>>> memo(N + 1);
        FOR(i, 0, N) {
            dp[i][1] = 1;
            FOR(l, 1, N + 1) {
                dp[i][l] += memo[l - 1][A[i] - d];
                memo[l - 1][A[i]] += dp[i][l - 1];
            }
        }
        FOR(i, 0, N) {
            FOR(l, 2, N + 1) {
