...
int main() {
    ...
    LL ans = 0;
    vector<vector<pair<int, LL>>> events(N);
    FOR(i, 0, Q) {
        int L, R, C;
        cin >> L >> R >> C;
        L--;
        R--;
        events[L].emplace_back(R, C);
        ans += C;
    }

    MinPriorityQueue<pair<LL, int>> que;
    FOR(i, 0, N - 1) {
        for (auto [r, c] : events[i]) {
            que.emplace(c, r);
        }

        while (not que.empty() and que.top().second <= i) {
            que.pop();
        }
