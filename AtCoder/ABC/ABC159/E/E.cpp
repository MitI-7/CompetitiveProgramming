...
LL solve(const vector<vector<int>> &board, vector<int> cut, LL K) {
    LL H = LEN(board);
    LL W = LEN(board[0]);

    cut.emplace_back(H - 1);

    LL ans = 0;

    vector<int> num(LEN(cut));

    FOR(x, 0, W) {
        int pre = 0;
        vector<int> now(LEN(cut));
        FOR(i, 0, LEN(cut)) {
            int y1 = pre;
            int y2 = cut[i] + 1;
            FOR(y, y1, y2) {
...
int main() {
    ...
    for (int mask = 0; mask < (1 << (H - 1)); ++mask) {
        auto cut = make_v<int>(0);
        for (int idx = 0; idx < H; ++idx) {
            if (is_bit_on(mask, idx)) {
                cut.emplace_back(idx);
            }
        }
        chmin(ans, solve(board, cut, K));
    }
    print(ans);
