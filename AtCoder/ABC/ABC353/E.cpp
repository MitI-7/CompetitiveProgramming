...
class Node {
public:
    int count;          // このノードが終点となる回数
    int count_throw;    // このノードを通る回数
    std::array<std::unique_ptr<Node>, 27> child;

    Node() : count(0), count_throw(0) {}
};
...
public:
    ...
    // s を num 個挿入する
    void insert(const std::string &s, const int num = 1) {
        auto node = this->root.get();

        for (const auto c : s) {
            if (node->child[c - 'a'] == nullptr) {
                node->child[c - 'a'] = std::make_unique<Node>();
            }
            node = node->child[c - 'a'].get();
            node->count_throw += num;
        }
        node->count += num;
    }
    ...
    int count_throw(const std::string &s) const {
        auto node = this->root.get();
        for (const auto c : s) {
            if (node->child[c - 'a'] == nullptr) {
                return 0;
        ...
        return node->count_throw;
    }

    int count_throw_total(const std::string &s) const {
        auto node = this->root.get();
        int total = 0;
        for (const auto c : s) {
            if (node->child[c - 'a'] == nullptr) {
