...
public:
    TopologicalSort(int num_nodes) : num_nodes(num_nodes) {
        ...
        this->indegree.resize(num_nodes, 0);
        this->used.resize(num_nodes, false);
    }
    ...
    bool sort() {
        ...
        for (int u = 0; u < this->num_nodes; ++u) {
            if (this->indegree[u] == 0 and not this->used[u]) {
                this->bfs(u);
            }
        }
...
private:
    void bfs(const int s) {
        ...
        while (not que.empty()) {
            ...
            for (int v : this->graph[u]) {
                this->indegree[v]--;
                if (this->indegree[v] == 0 and not this->used[v]) {
                    this->used[v] = true;
                    que.push(v);
                }
            }
...
int main() {
    ...
    // 1 回で T が S を 1 周しないようにしておく
    while (LEN(S) < N + M) {
        S += S;
    }
    RollingHash rh_s(S), rh_t(T);
    const auto hash = rh_t.hash(0, M);

    TopologicalSort ts(N);
    FOR(left, 0, N) {
        auto right = left + M;
        if (rh_s.hash(left, right) == hash) {
            // 自明なループ
            if (left == right % N) {
                print(-1);
                return 0;
            }
            ts.add_directed_edge(left, right % N);
        }
    }
