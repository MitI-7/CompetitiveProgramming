...
int main() {
    ...
    auto dp1 = make_v<LL>((1 << half) + 100);
    {
        FOR(b, 0, (1 << half)) {
            FOR(i, 0, min(half, N)) {
                if (not is_bit_on(b, i)) {
                    auto new_b = bit_set(b, i, 1);
                    dp1[new_b] = dp1[b] + A[i];
                }
            }
        }
    ...
    time.emplace(0);
    {
        auto dp2 = make_v<LL>((1 << half) + 100);
        FOR(b, 0, (1 << half)) {
            FOR(i, half, N) {
                if (not is_bit_on(b, i - half)) {
                    auto new_b = bit_set(b, i - half, 1);
                    dp2[new_b] = dp2[b] + A[i];
                }
            }
        }
