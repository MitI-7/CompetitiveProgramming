...
int main() {
    ...
    FOR(y, 0, H) {
        FOR(x, 0, W) {
            if (field[y][x] == 'S') {
                ...
                tel[field[y][x]].emplace_back(make_pair(y, x));
            }
        }
    }
    ...
    que.emplace(make_tuple(0, sy, sx));
    ...
    while (not que.empty()) {
        ...
        FOR(i, 0, 4) {
            ...
            if (inside(ny, nx, H, W) and field[ny][nx] != '#') {
                if (dp[ny][nx] > c + 1) {
        ...
        auto t = field[y][x];
        if (t != '#' and t != 'S' and t != 'G' and t != '.') {
            while (not tel[t].empty()) {
                auto p = tel[t].back(); tel[t].pop_back();
                if (p.first != y or p.second != x) {
                    if (dp[p.first][p.second] > c + 1) {
                        que.emplace(make_tuple(c + 1, p.first, p.second));
                    }
                }
            }
        }
