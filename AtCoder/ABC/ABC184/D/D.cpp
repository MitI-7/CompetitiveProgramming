...
int main() {
    ...
    auto dp = make_v<double>(110, 110, 110);
    dp[A][B][C] = 1.0;
    ...
    FOR(a, A, 100) {
        FOR(b, B, 100) {
            FOR(c, C, 100) {
                ...
                dp[a][b + 1][c] += dp[a][b][c] * (1.0 * b) / (a + b + c);
                dp[a][b][c + 1] += dp[a][b][c] * (1.0 * c) / (a + b + c);
            }
        }
    }
    ...
    FOR(i, 0, 100) {
        FOR(j, 0, 100) {
            ans += (100 + i + j - total) * dp[100][i][j];
            ans += (100 + i + j - total) *dp[i][100][j];
            ans += (100 + i + j - total) *dp[i][j][100];
        }
    }
