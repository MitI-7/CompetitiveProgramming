...
int main() {
    ...
    // dp[i][h] = i までみて，前回だした手がhのときの最大勝利数
    auto dp = make_v<int>(N + 1, 4);
    fill_v(dp, -INF);
    dp[0][3] = 0;

    FOR(i, 0, N) {
        const int win_h = (a[i] + 2) % 3;
        const int draw_h = a[i];
        const int lose_h = (a[i] + 1) % 3;

        FOR(pre_h, 0, 4) {
            FOR(new_h, 0, 3) {
                if (new_h == pre_h or new_h == lose_h) {
                    continue;
                }

                if (new_h == win_h) {
                    chmax(dp[i + 1][new_h], dp[i][pre_h] + 1);
                }
                if (new_h == draw_h) {
                    chmax(dp[i + 1][new_h], dp[i][pre_h]);
                }
            }
        }
    }
