...
int main() {
    ...
    // dp[i][j][k] = i段目までみて，j個保留して，スコアがkの場合の数
    auto dp = make_v<mint<MOD>>(N + 1, N + 1, N * N + 1);
    dp[0][0][0] = 1;
        ...
        FOR(j, 0, N) {
            FOR(k, 0, K + 1) {
                ...
                // i段同士をつなげる
                {
                    ...
                    dp[i + 1][j][new_score] += dp[i][j][k];
                }

                // i段は両方保留
                {
                    dp[i + 1][j + 1][new_score] += dp[i][j][k];
                }

                // 片方上とつなげる
                {
                    dp[i + 1][j][new_score] += dp[i][j][k] * (j * 2);
                }

                // 両方上とつなげる
                {
                    if (j - 1 >= 0) {
                        dp[i + 1][j - 1][new_score] += dp[i][j][k] * j * j;
                    }
                }
            }
        }
