...
template<typename T>
Tree<T> read_unweighted_tree(const int num_nodes) {
    Tree<T> tree(num_nodes);
    for (int i = 0; i < num_nodes - 1; ++i) {
        int u, v;
        std::cin >> u >> v;
        u--;
        v--;
        tree.add_undirected_edge(u, v, 0, i);
    }
    return tree;
}

template<typename T>
Tree<T> read_weighted_tree(const int num_nodes) {
    Tree<T> tree(num_nodes);
    ...
    for (int i = 0; i < num_nodes - 1; ++i) {
        ...
        u--;
        v--;
        tree.add_undirected_edge(u, v, w, i);
    }
    return tree;
}

vector<vector<pair<int, int>>> node_d_i;
vector<int> ans;

void dfs(int u, int depth, vector<int> &depth_count, Tree<int> &tree) {
    for (auto [d, i]: node_d_i[u]) {
        ans[i] -= depth_count[d];
    }

    depth_count[depth]++;
    FOE(e, tree[u]) {
        dfs(e.to, depth + 1, depth_count, tree);
    }
...
int main() {
    ...
    FOR(i, 0, Q) {
        ...
        cin >> U >> D;
        U--;
        node_d_i[U].emplace_back(D, i);
    }

    vector<int> depth_count(N + 10);
    dfs(0, 0, depth_count, tree);
    print(ans);
