...
template<typename T> std::vector<vector<T>> dijkstra(const int start, const Graph<T> &graph) {

    //[(最短距離, node番号)]のque(距離が近い順にとりだす)
    std::priority_queue<std::tuple<T, int, int>, std::vector<std::tuple<T, int, int>>, std::greater<std::tuple<T, int, int>>> que;
    que.push({0, start, 0});

    auto seen = make_v<bool>(graph.num_nodes, 4);
    auto distance = make_v<int>(graph.num_nodes, 4);
    fill_v(distance, INF);
    distance[start][0] = 0;

    while (not que.empty()) {
        const auto [now_dist, u, rest] = que.top(); que.pop();
        if (seen[u][rest]) {
            continue;
        }
        seen[u][rest] = true;
        ...
        for (const auto &edge : graph.graph[u]) {
            if (rest > 0) {
                if (distance[edge.v][rest - 1] > now_dist) {
                    distance[edge.v][rest - 1] = now_dist;
                    que.push({now_dist, edge.v, rest - 1});
                }
            }
            else {
                if (distance[edge.v][2] > now_dist + 1) {
                    distance[edge.v][2] = now_dist + 1;
                    que.push({now_dist + 1, edge.v, 2});
                }
            }
        }
    }
