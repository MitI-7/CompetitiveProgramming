...
template<class T> class MinusVector {
public:
    int mini;
    int maxi;
    int base;
    std::vector<T> array;
    MinusVector(T mini, T maxi, T init) : mini(mini), maxi(maxi), base(abs(mini) + 1) {
        array.resize(base + maxi, init);
    }
...
int main() {
    ...
    int H, W;
    cin >> H >> W;
    auto grid = make_v<pair<int, int>>(H, W);
    FOR(y, 0, H) {
        FOR(x, 0, W) {
            cin >> grid[y][x].first;
        }
    ...
    vector<vector<MinusVector<int>>> dp(H, vector<MinusVector<int>>(W, MinusVector<int>(-(160 * 80 + 10), (160 * 80 + 10), 0)));
    dp[0][0][+grid[0][0].first - grid[0][0].second] = 1;
    dp[0][0][-grid[0][0].first + grid[0][0].second] = 1;
    FOR(y, 0, H) {
        FOR(x, 0, W) {
            FOR(d, -(160 * 80 + 9), 160 * 80 + 10) {
                if (dp[y][x][d] == 0) {
                    continue;
                }
                ...
                // 下
                {
                    ...
                    int nx = x;
                    if (inside(ny, nx, H, W)) {
                        dp[ny][nx][d + grid[ny][nx].first - grid[ny][nx].second] = 1;
                        dp[ny][nx][d - grid[ny][nx].first + grid[ny][nx].second] = 1;
                    }
                }
