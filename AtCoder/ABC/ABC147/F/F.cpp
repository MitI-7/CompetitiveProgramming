...
int main() {
    ...
    FOR(k, 0, N + 1) {
        auto L = k * X + D * sum_of_arithmetic_progression(0, 1, k);
        auto R = sum_of_arithmetic_progression(N - k, 1, k) - sum_of_arithmetic_progression(0, 1, k);
        m[(L + base) % D].emplace_back(L, R);
    }
    ...
    FOE(p, m) {
        auto v = p.second;
        FOR(i,0, LEN(v)) {
            v[i].first = (v[i].first - p.first) / D;
        }
        auto dp = make_v<pair<LL, LL>>(0);
        FOR(i,0, LEN(v)) {
            dp.emplace_back(v[i].first, 1);
            dp.emplace_back(v[i].first + v[i].second + 1, -1);
        }
        sort(ALL(dp));
        LL now = 0, pre = -INF;
        FOE(p2, dp) {
