...
std::set<long long> get_divisor(long long n) {
...
std::unordered_map<long long, long long> prime_factor_decomposition(long long n) {
...
int main() {
    ...
    auto ret1 = get_divisor(A);
    auto ret2 = get_divisor(B);
    set<LL> ret;
    set_intersection(ALL(ret1), ALL(ret2), inserter(ret, ret.begin()));
    LL ans = 0;
    set<LL> use_p;
    FOE(v, ret) {
        bool ok = true;
        ...
        if (ok) {
            ans++;
            auto m = prime_factor_decomposition(v);
            FOE(p, m) {
                use_p.insert(p.first);
            }
        }
    }
