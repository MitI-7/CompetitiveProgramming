...
int main() {
    ...
    FOR(i, 0, Q) {
        ...
        auto ok = [&](const int n) {
            ...
            const int idx = freq.kth_x(1, n - 1, L, R);
            if (idx == -1) {
                return false;
            }
            const int idx2 = freq.next(0, idx);
            if (idx2 == -1) {
                return false;
            }
            return freq.count_range(2, idx2, R) >= n;
        };
