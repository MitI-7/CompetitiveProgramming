...
int main() {
    ...
    // dp[bit] = bitを使用済みで，次にみれる最小のインデックス
    auto dp = make_v<int>(1 << 20);
    fill_v(dp, INF);
    dp[0] = 0;
    ...
    FOR(b, 0, 1 << 20) {
        ...
        FOR(c, 0, 20) {
            ...
            const int idx2 = srf.next(c, idx);
            if (idx2 == -1) {
                continue;
            }
            const int idx3 = srf.next(c, idx2 + 1);
            if (idx3 == -1) {
                continue;
            }

            chmin(dp[get_bit_set(b, c, 1)], idx3 + 1);
        }
    }
