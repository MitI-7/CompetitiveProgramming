...
std::vector<std::pair<int, int>> run_length_encoding(const std::vector<int> &A) {
    ...
    for (int i = 0; i < n - 1; ++i) {
        if (A[i] == A[i + 1]) {
            ...
            ans.emplace_back(A[i], num);
            num = 1;
        }
    }
    ans.emplace_back(A.back(), num);
...
int main() {
    ...
    const auto rle = run_length_encoding(A);
    ...
    FOE(v, vs) {
        ...
        for (int l = 0; l < n; ++l) {
            // rを進められるだけ進める
            while (r < n and not used.contains(v[r])) {
                used.insert(v[r]);
                ++r;
            }
            chmax(ans, r - l);
