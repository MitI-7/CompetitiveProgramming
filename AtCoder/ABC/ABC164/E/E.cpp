...
int main(int argc, char *argv[]) {
    ...
    auto graph = make_v<tuple<int, LL, LL>>(N, 0);
    FOR(i, 0, M) {
        LL U, V, A, B;
        cin >> U >> V >> A >> B;
        U--; V--;
    ...
    auto change = make_v<pair<LL, LL>>(N);
    FOR(i, 0, N) {
        cin >> change[i].first >> change[i].second;
    }

    auto dp = make_v<LL>(N, 3100);
    fill_v(dp, LINF);

    priority_queue<tuple<LL, LL, LL>, vector<tuple<LL, LL, LL>>, greater<tuple<LL, LL, LL>>> que;
    que.emplace(make_tuple(0, -min(3000LL, S), 0));

    while (not que.empty()) {
        LL t, m, u;
        tie(t, m, u) = que.top(); que.pop();
        m *= -1;
        ...
        // 両替
        {
            auto p = change[u];
            if (m + p.first < 3000) {
                if (dp[u][m + p.first] > t + p.second) {
                    que.emplace(make_tuple(t + p.second, -(m + p.first), u));
                }
            }
        }
        ...
        FOE(tu, graph[u]) {
            ...
            if (m >= a and dp[v][m - a] > t + b) {
                que.emplace(make_tuple(t + b, -(m - a), v));
            }
        }
    }
