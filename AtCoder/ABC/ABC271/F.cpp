...
// 2次元配列を右に90度回転
std::vector<std::vector<int>> rotate_matrix(const std::vector<std::vector<int>> &v) {
    std::vector<std::vector<int>> res(v[0].size(), std::vector<int>(v.size(), 0));
...
LL solve(const int N, vector<vector<int>> &grid) {

    auto dp1 = make_v<int>(N, N, 0);
    dp1[0][0].emplace_back(grid[0][0]);
    FOR(y, 0, N) {
        FOR(x, 0, N) {
            if (y + x >= N) {
                continue;
            ...
            FOR(i, 0, 2) {
                ...
                if (inside(ny, nx, N, N)) {
                    FOE(a, dp1[y][x]) {
                        dp1[ny][nx].emplace_back(a ^ grid[ny][nx]);
                    }
                }
            }
        }
    ...
    grid = rotate_matrix(rotate_matrix(grid));

    auto dp2 = make_v<int>(N, N, 0);
    dp2[0][0].emplace_back(grid[0][0]);
    FOR(y, 0, N) {
        FOR(x, 0, N) {
            if (y + x >= N) {
                continue;
            ...
            FOR(i, 0, 2) {
                ...
                if (inside(ny, nx, N, N)) {
                    FOE(a, dp2[y][x]) {
                        dp2[ny][nx].emplace_back(a ^ grid[ny][nx]);
                    }
                }
            }
        }
