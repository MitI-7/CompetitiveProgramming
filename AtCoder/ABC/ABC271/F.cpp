...
int main() {
    ...
    auto dp1 = make_v<map<int, LL>>(N, N);
    dp1[0][0][A[0][0]] = 1;
    ...
    FOR(y, 0, N) {
        FOR(x, 0, N) {
            ...
            FOE(p, dp1[y][x]) {
                if (inside(y, x + 1, N, N)) {
                    dp1[y][x + 1][p.first ^ A[y][x + 1]] += p.second;
                }
                if (inside(y + 1, x, N, N)) {
                    dp1[y + 1][x][p.first ^ A[y + 1][x]] += p.second;
                }
            }
        }
    }

    auto dp2 = make_v<map<int, LL>>(N, N);
    ...
    for (int y = N - 1; y >= 0; --y) {
        for (int x = N - 1; x >= 0; --x) {
            ...
            FOE(p, dp2[y][x]) {
                if (inside(y, x - 1, N, N)) {
                    dp2[y][x - 1][p.first ^ A[y][x - 1]] += p.second;
                }
                if (inside(y - 1, x, N, N)) {
                    dp2[y - 1][x][p.first ^ A[y - 1][x]] += p.second;
                }
            }
    ...
    FOR(y, 0, N) {
        int x = N - y - 1;
        FOE(p, dp1[y][x]) {
            ans += dp2[y][x][p.first ^ A[y][x]] * p.second;
        }
    }
    print(ans);
