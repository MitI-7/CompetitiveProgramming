...
mint<MOD> sum(LL h, LL w, LL upper_left, LL M) {
    mint<MOD> bottom_right = upper_left + (h - 1) * M * 2 + (w - 1) * 2;
    mint<MOD> average = mint<MOD>(upper_left) + bottom_right;
    return average * h * w / 2;
}

mint<MOD> func(LL y, LL x, LL M) {
    mint<MOD> ans;
    // 奇数行だけとりだしたやつ
    ans += sum((y + 1) / 2, (x + 1) / 2, 1, M);
    // 偶数行だけとりだしたやつ
    ans += sum(y / 2, x / 2, M + 2, M);
