...
LL solve() {
    ...
    while (LEN(v) <= L) {
        ...
        if (i + 1 < N and not used.contains({i + 1, j})) {
            used.insert({i + 1, j});
            que.emplace(c - A[i].first + A[i + 1].first, i + 1, j);
        }
        if (j + 1 < M and not used.contains({i, j + 1})) {
            used.insert({i, j + 1});
            que.emplace(c - B[j].first + B[j + 1].first, i, j + 1);
        }
    }

    assert(LEN(v) > L);
    for (auto [c, i, j]: v) {
        if (ng.contains({A[i].second, B[j].second})) {
            continue;
        }
        print(c);
        return 0;
