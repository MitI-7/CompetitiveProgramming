...
int main() {
    ...
    LL ans = LINF;
    FOE(const a, s) {
        // dp[y][x][num] = (y, x)にいて，aより大の値をとった回数が num のときの最小コスト
        auto dp = make_v<LL>(H, W, K + 2);
        fill_v(dp, LINF);
        if (A[0][0] > a) {
            dp[0][0][1] = A[0][0];
        } else {
        ...
        FOR(y, 0, H) {
            FOR(x, 0, W) {
                FOR(num, 0, K + 1) {
                    ...
                    if (inside(y, x + 1, H, W)) {
                        if (A[y][x + 1] > a) {
                            chmin(dp[y][x + 1][num + 1], dp[y][x][num] + A[y][x + 1]);
                        } else {
                            chmin(dp[y][x + 1][num], dp[y][x][num]);
                        }
                    }
                    ...
                    if (inside(y + 1, x, H, W)) {
                        if (A[y + 1][x] > a) {
                            chmin(dp[y + 1][x][num + 1], dp[y][x][num] + A[y + 1][x]);
                        } else {
                            chmin(dp[y + 1][x][num], dp[y][x][num]);
                        }
                    }
                }
            }
        ...
        FOR(num, 0, K + 1) {
            chmin(ans, dp[H - 1][W - 1][num] + (K - num) * a);
        }
    }
    print(ans);
