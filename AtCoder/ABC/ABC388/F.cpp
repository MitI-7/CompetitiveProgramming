...
bool solve(LL N, int A, int B, vector<pair<LL, LL>> &ngs) {
    if (A == B) {
        for (auto [l, r] : ngs) {
            LL x = (r - 1) / A * A;
            if (l <= x) {
    ...
    ngs.emplace_back(N, N);

    LL now = 0;
    vector<int> dp(B); // now の i マス前が到達可能かどうか
    dp[0] = 1;
    for (const auto [l, r] : ngs) {
        if (r - l >= B) {
            return false;
        }
...
int main() {
    ...
    vector<pair<LL, LL>> ngs(M);
    FOR(i, 0, M) {
        cin >> ngs[i].first >> ngs[i].second;
        ngs[i].first--;
    }
