...
int main() {
    ...
    auto dp = make_v<int>(N + 1, 510, 510);
    fill_v(dp, INF);
    dp[0][0][0] = 0;
    ...
    FOR(i, 0, N) {
        FOR(a, 0, 510) {
            FOR(b, 0, 510) {
                ...
                // aにいれる
                if (a + B[i] < 510) {
                    chmin(dp[i + 1][a + B[i]][b], dp[i][a][b] + (A[i] != 0));
                }
                // bにいれる
                if (b + B[i] < 510) {
                    chmin(dp[i + 1][a][b + B[i]], dp[i][a][b] + (A[i] != 1));
                }
                // cにいれる
                chmin(dp[i + 1][a][b], dp[i][a][b] + (A[i] != 2));
            }
        }
    }

    int ans = dp[N][total / 3][total / 3];
    if (ans == INF) {
        ans = -1;
    }
    print(ans);
