...
int main() {
    ...
    vector<int> ng_edges;
    map<pair<int, int>, pair<int, int>> uv_edge;
    ...
    FOR(i, 0, Q) {
        ...
        if (T == 1) {
            ...
            cin >> I;
            I--;
            int node = N + LEN(ng_edges);
            const auto [u, v, _] = edges[I];
            uv_edge[{u, v}] = {I, node};
            queries.emplace_back(T, I, node);
            ng_edges.emplace_back(I);
        }
        else {
            int X, Y;
            cin >> X >> Y;
    ...
    const int N2 = LEN(ng_edges);
    auto matrix = make_v<LL>(N + N2, N + N2);
    fill_v(matrix, LINF);
    FOR(u, 0, N + N2) {
        matrix[u][u] = 0;
    }
    ...
    for (auto [u, v, c] : edges) {
        if (uv_edge.contains({u, v})) {
            auto [i, no] = uv_edge[{u, v}];
            matrix[u][no] = matrix[no][u] = c;
            matrix[no][v] = matrix[v][no] = 0;
        }
        else {
    ...
    const int num_nodes = N + N2;
    FOR(m, 0, N) {
        for (int s = 0; s < num_nodes; ++s) {
            for (int e = 0; e < num_nodes; ++e) {
                // s -> m -> e
                if (matrix[s][m] != LINF and matrix[m][e] != LINF) {
