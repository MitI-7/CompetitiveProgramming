...
int main() {
    ...
    FOR(i, 0, Q) {
        ...
        do {
            vector<pair<int, LL>> now = {{0, 0}, {0, 0}};
            FOE(b, B) {
                auto [u, v, t] = edges[b];

                LL u_cost = LINF, v_cost = LINF;
                for (auto [a, c] : now) {
                    chmin(u_cost, c + matrix[a][v] + t);
                    chmin(v_cost, c + matrix[a][u] + t);
                }
                now = {{u, u_cost}, {v, v_cost}};
            }
            chmin(ans[i], now[0].second + matrix[now[0].first][N - 1]);
            chmin(ans[i], now[1].second + matrix[now[1].first][N - 1]);
