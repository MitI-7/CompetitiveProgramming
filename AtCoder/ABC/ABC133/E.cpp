...
template<typename T> void dfs_children(const int u, const int p, const int d, std::vector<int> &children, const Tree<T> &tree) {
    children[u] = 0;
    for (auto &edge : tree.graph[u]) {
        if (edge.v != p) {
            dfs_children(edge.v, u, d + 1, children, tree);
            children[u]++;
        }
    }
}

// 木の各ノードを頂点とする子供の数を求める
// O(n)
template<typename T> std::vector<int> tree_children(const int root, const Tree<T> &tree) {
    std::vector<int> children(tree.num_nodes, -1);
    dfs_children(root, -1, 0, children, tree);
    return children;
}
...
int main() {
    ...
    auto num_children = tree_children(0, tree);
    mint<MOD> ans = K;
    FOR(u, 1, N) {
        FOR(i, 2, num_children[u] + 2) {
            if (K - i <= 0) {
                print(0);
                return 0;
            }
