...
int main() {
    ...
    vector<int> leader_count_a(N), leader_count_b(N);
    FOR(i, 0, K) {
        int A;
        cin >> A;
        A--;
    ...
    for (const auto& [w, u, v] : edges) {
        ...
        const int g1 = uf.leader(u);
        const int g2 = uf.leader(v);

        const int g1_a = leader_count_a[g1];
        const int g2_a = leader_count_a[g2];
        const int g1_b = leader_count_b[g1];
        const int g2_b = leader_count_b[g2];
        leader_count_a[g1] = leader_count_a[g2] = leader_count_b[g1] = leader_count_b[g2] = 0;
        uf.unite(g1, g2);
        const int g3 = uf.leader(g1);

        // g1のaとg2のbをマッチ
        if (g1_a >= g2_b) {
            ans += g2_b * w;
            leader_count_a[g3] += g1_a - g2_b;
        }
        else {
            ans += g1_a * w;
            leader_count_b[g3] += g2_b - g1_a;
        }

        // g2のaとg1のbをマッチ
        if (g2_a >= g1_b) {
            ans += g1_b * w;
            leader_count_a[g3] += g2_a - g1_b;
        }
        else {
            ans += g2_a * w;
            leader_count_b[g3] += g1_b - g2_a;
        }
    }
    print(ans);
