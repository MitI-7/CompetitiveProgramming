...
class CumulativeSum3D {
    int N;
    std::vector<std::vector<std::vector<T>>> memo;
public:
    CumulativeSum3D(int n, const std::vector<std::vector<std::vector<T>>>& A) {
        N = n;
        memo.assign(N + 1, std::vector<std::vector<T>>(N + 1, std::vector<T>(N + 1, 0)));

        // 3D累積和を構築
        for (int i = 1; i <= N; ++i) {
            for (int j = 1; j <= N; ++j) {
                for (int k = 1; k <= N; ++k) {
                    memo[i][j][k] = A[i - 1][j - 1][k - 1]
                                  + memo[i - 1][j][k] + memo[i][j - 1][k] + memo[i][j][k - 1]
                                  - memo[i - 1][j - 1][k] - memo[i - 1][j][k - 1] - memo[i][j - 1][k - 1]
                                  + memo[i - 1][j - 1][k - 1];
                }
            }
        }
    ...
    T query(int x1, int y1, int z1, int x2, int y2, int z2) {
        return memo[x2][y2][z2]
             - (x1 > 0 ? memo[x1 - 1][y2][z2] : 0)
             - (y1 > 0 ? memo[x2][y1 - 1][z2] : 0)
             - (z1 > 0 ? memo[x2][y2][z1 - 1] : 0)
             + (x1 > 0 && y1 > 0 ? memo[x1 - 1][y1 - 1][z2] : 0)
             + (x1 > 0 && z1 > 0 ? memo[x1 - 1][y2][z1 - 1] : 0)
             + (y1 > 0 && z1 > 0 ? memo[x2][y1 - 1][z1 - 1] : 0)
             - (x1 > 0 && y1 > 0 && z1 > 0 ? memo[x1 - 1][y1 - 1][z1 - 1] : 0);
    }
};
...
int main() {
    ...
    FOR(i, 0, Q) {
        ...
        ans[i] = cs.query(LX, LY, LZ, RX, RY ,RZ);
    }
    print(ans);
