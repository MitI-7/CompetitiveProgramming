...
int main() {
    ...
    reverse(ALL(S));
    S += "0";
    // dp[i][j] = i桁までみたとき，繰り上がり要求があるjようなときの最小数
    auto dp = make_v<LL>(LEN(S) + 10, 2);
    fill_v(dp, INF);
    dp[0][0] = 0;
    ...
    FOR(i, 0, LEN(S)) {
        ...
        FOR(d, 0, 9) {
            if (c <= d) {
                chmin(dp[i + 1][0], dp[i][0] + d + (d - c));
            }
            else {
                chmin(dp[i + 1][1], dp[i][0] + d + (10 - c));
            }

            if (c + 1 <= d) {
                chmin(dp[i + 1][0], dp[i][1] + d + (d - c) - 1);
            }
            else {
                chmin(dp[i + 1][1], dp[i][1] + d + (10 - c) - 1);
            }
        }
    }
    print(dp[LEN(S)][0]);
