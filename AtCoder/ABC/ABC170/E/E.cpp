...
int main() {
    ...
    LL N, Q;
    cin >> N >> Q;
    auto best = make_v<int>(200010);
    vector<priority_queue<pair<int, int>>> v_que(200010);
    auto syozoku = make_v<int>(N);
    auto A = make_v<int>(N);
    FOR(i, 0, N) {
        LL B;
        cin >> A[i] >> B;
        ...
        chmax(best[B], A[i]);
        v_que[B].emplace(make_pair(A[i], i));
        syozoku[i] = B;
    }

    SegmentTree<int> seg(200010, SegmentTree<int>::RangeMinimumQuery);
    FOR(i, 0, LEN(best)) {
        if (best[i] == 0) {
            seg.update(i, INF);
        }
    ...
    vector<unordered_set<int>> v_rem(200010);
    FOR(i, 0, Q) {
        LL C, D;
        cin >> C >> D;
        C--; D--;

        // 園児Cが幼稚園fromから幼稚園Dに行く
        const int from = syozoku[C];
        v_rem[from].insert(C);

        // 園児Cを幼稚園fromから取り除く
        if (not v_que[from].empty() and v_que[from].top().second == C) {
            v_que[from].pop();

            while (not v_que[from].empty() and v_rem[from].find(v_que[from].top().second) != v_rem[from].end()) {
                v_que[from].pop();
            }
            ...
            if (v_que[from].empty()) {
                ...
                seg.update(from, v_que[from].top().first);
            }
        }

        // 園児Cを幼稚園Dに入れる
        syozoku[C] = D;
        v_que[D].emplace(make_pair(A[C], C));
        if (v_rem[D].find(C) != v_rem[D].end()) {
            v_rem[D].erase(C);
        }

        if (v_que[D].top().second == C) {
            seg.update(D, A[C]);
        }

        print(seg.query(0, 200001));
    }
