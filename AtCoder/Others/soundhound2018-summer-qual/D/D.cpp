...
vector<LL> calc_cost(LL start, bool use_a, vector<vector<tuple<LL, LL, LL>>> &graph) {
    int N = LEN(graph);
    auto cost = make_v<LL>(N);
    fill_v(cost, LINF);
    priority_queue<pair<LL, LL>, vector<pair<LL, LL>>, greater<pair<LL, LL>>> que;

    que.emplace(make_pair(0, start));

    while (not que.empty()) {
        LL c, u;
        tie(c, u) = que.top(); que.pop();
        ...
        FOE(t, graph[u]) {
            ...
            if (use_a) {
                if (cost[v] > c + a) {
                    que.emplace(make_pair(c + a, v));
                }
            }
            else {
                if (cost[v] > c + a) {
                    que.emplace(make_pair(c + b, v));
                }
            }
        }
    }
...
int main() {
    ...
    auto graph = make_v<tuple<LL, LL, LL>>(N, 0);
    FOR(i, 0, M) {
        LL U, V, A, B;
        cin >> U >> V >> A >> B;
        U--; V--;
        graph[U].emplace_back(make_tuple(V, A, B));
        graph[V].emplace_back(make_tuple(U, A, B));
    }

    auto cost_yen = calc_cost(S, true, graph);
    auto cost_sun = calc_cost(T, false, graph);


    SegmentTree<LL> seg(N, SegmentTree<LL>::RangeMaximumQuery);
    FOR(u, 0, N) {
        seg.update(u, 1000000000000000LL - cost_yen[u] - cost_sun[u]);
    }
